...
Public Sub PivotBuilder_Rebuild()
    ...
    ' 1. FLATTEN DATA SILENTLY (Fix for Finding 2: Graceful Exit)
...
Private Function AutoFlattenData() As Boolean
    ...
    AutoFlattenData = True ' Assume success by default
    ...
    ' Fix for Finding 2: Graceful failure instead of executing 'End'
    ...
    For i = 2 To lastRow
        ...
        bldRaw = Replace(bldRaw, Chr(160), "")
         
        ' Fix for Finding 3: Fallback assignment prevents data starvation
        ...
        If InStr(tradeRaw, "/") > 0 Then
            ...
            arrOut(outRow, colBld) = bldRaw ' Inject fallback building
...
Private Function RecreatePivotSheet_safe(ByVal sheetName As String) As Worksheet
    ...
    If ws Is Nothing Then
        ...
        On Error GoTo 0
    Else
        ' MUST explicitly clear tables to release the ghost caches from memory
    ...
    ' Purge unattached caches workbook-wide
    For Each pc In ThisWorkbook.PivotCaches
        If pc.MissingItemsLimit = xlMissingItemsNone Then
            On Error Resume Next
            pc.MissingItemsLimit = xlMissingItemsDefault
            On Error GoTo 0
        End If
    Next pc
...
Private Function BuildVerticalPivot_safe( _
    ...
    With pt
        .HasAutoFormat = False
        .NullString = "-"
        .CompactLayoutRowHeader = "Building Summary"
        ...
        ' --- Add Estimated FTEs to the PivotTable ---
        .CalculatedFields.Add "Estimated FTEs", "='" & FLD_TOTAL_HOURS & "' / 2080"
        With .PivotFields("Estimated FTEs")
            .Orientation = xlDataField
            .Function = xlSum
            .NumberFormat = "#,##0.00"
        End With
    ...
    pt.PivotFields(FLD_SPECIFIC_TRADE).PivotItems("(blank)").Visible = False
    
    ' Explicitly loop through and collapse every building group
    For Each pItem In pt.PivotFields(FLD_BUILDING).PivotItems
        pItem.ShowDetail = False
    Next pItem
    On Error GoTo 0
...
Private Sub WriteTotalsPanel_FromSource(ByVal pt As PivotTable, ByVal WsOut As Worksheet, ByVal StartCell As Range, ByVal WsSrc As Worksheet, ByVal SrcData As Range, ByVal FldZone As String, ByVal FldBuilding As String, ByVal FldHours As String, ByVal FldCost As String)
    Dim r As Long, c1 As Long, c2 As Long, c3 As Long, c4 As Long
    Dim zoneFilter As String
    Dim dict As Object, tradeDict As Object, alertDict As Object, bldgHasBackflow As Object
    Dim i As Long
    Dim colZone As Long, colBld As Long, colH As Long, colC As Long, colWho As Long
    Dim colM1 As Long, colM2 As Long, colFeas As Long, colAsset As Long
    ...
    colFeas = FindHeaderCol(SrcData, "PM Feasibility")
    colAsset = FindHeaderCol(SrcData, "Asset Sub Type")
    ...
    Set alertDict = CreateObject("Scripting.Dictionary")
    alertDict.CompareMode = 1
    Set bldgHasBackflow = CreateObject("Scripting.Dictionary")
    bldgHasBackflow.CompareMode = 1
    ...
    For i = 2 To SrcData.Rows.count
        ...
        If (zoneFilter = "") Or (StrComp(z, zoneFilter, vbTextCompare) = 0) Then
            ...
            If Len(b) > 0 And LCase$(b) <> "(blank)" Then
                ...
                ' --- 2. TALLY INDIVIDUAL CONTRACTOR HOURS ---
                If colWho > 0 Then
                    ...
                    If InStr(tradeRaw, "/") > 0 Then
                        ...
                        If InStr(1, t1, "Contractor", vbTextCompare) > 0 Then
                            Dim key1 As String: key1 = b & "|" & t1
                            If Not tradeDict.Exists(key1) Then tradeDict.Add key1, 0#
                            tradeDict(key1) = tradeDict(key1) + h1
                        End If
                         
                        If InStr(1, t2, "Contractor", vbTextCompare) > 0 Then
                            Dim key2 As String: key2 = b & "|" & t2
                            If Not tradeDict.Exists(key2) Then tradeDict.Add key2, 0#
                            tradeDict(key2) = tradeDict(key2) + h2
                        End If
                    Else
                        If InStr(1, tradeRaw, "Contractor", vbTextCompare) > 0 Then
                            Dim singleKey As String
                            singleKey = b & "|" & tradeRaw
                            If Not tradeDict.Exists(singleKey) Then tradeDict.Add singleKey, 0#
                            tradeDict(singleKey) = tradeDict(singleKey) + h
                        End If
                    End If
                End If
                
                ' --- 3. AUDIT BUILDING FOR BACKFLOW PREVENTERS ---
                If Not bldgHasBackflow.Exists(b) Then bldgHasBackflow.Add b, False
                
                Dim assetText As String, rawText As String
                If colAsset > 0 Then assetText = LCase$(Trim$(SafeStr(SrcData.Cells(i, colAsset).Value))) Else assetText = ""
                rawText = LCase$(Trim$(SafeStr(SrcData.Cells(i, 1).Value)))
                
                If InStr(1, assetText, "backflow", vbTextCompare) > 0 Or _
                   InStr(1, assetText, "back flow", vbTextCompare) > 0 Or _
                   InStr(1, rawText, "backflow", vbTextCompare) > 0 Or _
                   InStr(1, rawText, "back flow", vbTextCompare) > 0 Then
                    
                    bldgHasBackflow(b) = True
                End If
            End If
            
            ' --- 4. CATCH EXISTING PARENT ALERTS (Strict Match) ---
            If colFeas > 0 Then
                Dim fAlert As String
                fAlert = Trim$(SafeStr(SrcData.Cells(i, colFeas).Value))
                
                If InStr(1, fAlert, "Missing Parent", vbTextCompare) > 0 Then
                    fAlert = Replace(fAlert, "??", "")
                    fAlert = Replace(fAlert, ChrW(&H26A0), "")
                    fAlert = Trim$(fAlert)
                    fAlert = ChrW(&H26A0) & " " & fAlert
                    
                    Dim bAlert As String
                    If Len(b) = 0 Or LCase$(b) = "(blank)" Then bAlert = "(Unassigned)" Else bAlert = b
                    
                    Dim aKey As String
                    aKey = bAlert & "|" & fAlert
                    
                    If Not alertDict.Exists(aKey) Then
                        alertDict.Add aKey, 1
                    Else
                        alertDict(aKey) = alertDict(aKey) + 1
                    End If
                End If
            End If
        End If
    ...
    ' --- 5. INJECT BACKFLOW ALERTS FOR FAILED AUDITS ---
    Dim bk As Variant
    For Each bk In bldgHasBackflow.keys
        If bldgHasBackflow(bk) = False Then
            Dim bfKey As String
            bfKey = CStr(bk) & "|" & ChrW(&H26A0) & " Missing Building Backflow Device"
            alertDict.Add bfKey, 1
        End If
    Next bk
    ...
    With WsOut
        ...
        .Cells(r, c4).Value = "Total PM Material Cost"
        .Range(.Cells(r, c1), .Cells(r, c4)).Font.Bold = True
        .Range(.Cells(r, c1), .Cells(r, c4)).Interior.Color = RGB(242, 242, 242)
        r = r + 1
    End With
    ...
    For Each k In keys
        v = dict(k)
        With WsOut
            .Cells(r, c1).NumberFormat = "@" ' Force Text formatting
            .Cells(r, c1).Value = CStr(k)
            .Cells(r, c1).HorizontalAlignment = xlLeft ' Force left alignment
            .Cells(r, c2).Value = v(0): .Cells(r, c2).NumberFormat = "#,##0.00"
            .Cells(r, c3).Value = v(0) / 2080: .Cells(r, c3).NumberFormat = "#,##0.00"
            .Cells(r, c4).Value = v(1): .Cells(r, c4).NumberFormat = "$#,##0.00"
            .Range(.Cells(r, c1), .Cells(r, c4)).Font.Bold = True
            .Range(.Cells(r, c1), .Cells(r, c4)).Interior.Color = RGB(242, 242, 242)
        End With
        r = r + 1
    ...
    With WsOut
        ...
        .Cells(r, c3).Value = grandH / 2080: .Cells(r, c3).NumberFormat = "#,##0.00"
        .Cells(r, c4).Value = grandC: .Cells(r, c4).NumberFormat = "$#,##0.00"
        .Range(.Cells(r, c1), .Cells(r, c4)).Font.Bold = True
        .Range(.Cells(r, c1), .Cells(r, c4)).Interior.Color = RGB(217, 217, 217)
        With .Range(.Cells(r, c1), .Cells(r, c4)).Borders(xlEdgeTop)
    ...
    ' --- WRITE CONTRACTOR BREAKDOWN ---
    If tradeDict.count > 0 Then
        With WsOut
            .Cells(r, c1).Value = "Individual Contractor Breakdown (by Building)"
            .Cells(r, c1).Font.Bold = True
            r = r + 1
             
            .Cells(r, c1).Value = "Building"
            .Cells(r, c2).Value = "Contractor"
            .Cells(r, c3).Value = "Total Hours"
            .Cells(r, c4).Value = "Estimated FTEs"
            .Range(.Cells(r, c1), .Cells(r, c4)).Font.Bold = True
            .Range(.Cells(r, c1), .Cells(r, c4)).Interior.Color = RGB(242, 242, 242)
            r = r + 1
        End With
        ...
        Dim prevBldg As String
        prevBldg = ""
         
        For Each tKey In tKeys
            Dim partsTrade() As String
            partsTrade = Split(CStr(tKey), "|")
            
            With WsOut
                .Cells(r, c1).NumberFormat = "@" ' Force Text formatting
                .Cells(r, c1).Value = partsTrade(0) ' Building
                .Cells(r, c1).HorizontalAlignment = xlLeft ' Force left alignment
                
                .Cells(r, c2).Value = partsTrade(1) ' Contractor
                .Cells(r, c3).Value = tradeDict(tKey)
                .Cells(r, c3).NumberFormat = "#,##0.00"
                .Cells(r, c4).Value = tradeDict(tKey) / 2080
                .Cells(r, c4).NumberFormat = "#,##0.00"
                .Range(.Cells(r, c1), .Cells(r, c4)).Interior.Color = RGB(242, 242, 242)
                
                ' Add a separator line when the building changes
                If prevBldg <> "" And partsTrade(0) <> prevBldg Then
                    With .Range(.Cells(r, c1), .Cells(r, c4)).Borders(xlEdgeTop)
                        .LineStyle = xlContinuous
                        .weight = xlThin
                        .Color = RGB(150, 150, 150)
                    End With
                End If
                prevBldg = partsTrade(0)
            End With
            r = r + 1
        Next tKey
        
        ' Add a final bottom border to close off the box
        With WsOut.Range(WsOut.Cells(r - 1, c1), WsOut.Cells(r - 1, c4)).Borders(xlEdgeBottom)
            .LineStyle = xlContinuous
            .weight = xlThin
            .Color = RGB(150, 150, 150)
        End With
    ...
    ' --- WRITE MISSING PARENT ALERTS ---
    If alertDict.count > 0 Then
        With WsOut
            .Cells(r, c1).Value = "Hierarchy Audit: Missing Parent Assets"
            .Cells(r, c1).Font.Bold = True
            .Cells(r, c1).Font.Color = RGB(192, 0, 0)
            r = r + 1
            
            .Cells(r, c1).Value = "BUILDING"
            .Cells(r, c2).Value = "MISSING PARENT ALERT"
            .Cells(r, c4).Value = "CHILDREN IMPACTED"
            
            .Range(.Cells(r, c1), .Cells(r, c4)).Font.Bold = True
            .Range(.Cells(r, c1), .Cells(r, c4)).Font.Color = vbWhite
            .Range(.Cells(r, c1), .Cells(r, c4)).Interior.Color = RGB(0, 112, 192)
            r = r + 1
            
            Dim aKeys As Variant, aKeyItem As Variant
            aKeys = alertDict.keys
            If alertDict.count > 1 Then QuickSortText aKeys, LBound(aKeys), UBound(aKeys)
            
            For Each aKeyItem In aKeys
                Dim parts() As String
                parts = Split(aKeyItem, "|")
                
                .Cells(r, c1).NumberFormat = "@" ' Force Text formatting
                .Cells(r, c1).Value = CStr(parts(0))
                .Cells(r, c1).HorizontalAlignment = xlLeft ' Force left alignment
                
                .Cells(r, c2).Value = CStr(parts(1))
                .Cells(r, c4).Value = alertDict(aKeyItem)
                .Cells(r, c4).NumberFormat = "0"
                
                .Range(.Cells(r, c1), .Cells(r, c4)).Interior.Color = RGB(255, 204, 204)
                r = r + 1
            Next aKeyItem
        End With
    End If
    
    WsOut.Range(WsOut.Cells(StartCell.Row, c1), WsOut.Cells(r, c4)).Columns.AutoFit
End Sub
