...
HIERARCHY_RULES = [
    ...
    {
        # --- BUG 2 FIXED: Fire Pumps correctly mapped as Parents ---
...
def load_knowledge_base():
    # We declare globals so we can overwrite them at the end safely
    ...
    # Temp variables to prevent race conditions during load
                ...
                for line in f:
                    ...
                    if len(parts) >= 3:
                        ...
                        if rule_type in ["alias", "phrase", "list"]:
                            ...
                            t_broad_categories.append(from_text)
    ...
    # ATOMIC SWAP: Safely overwrite globals with lock
...
PM_SUITABILITY_RULES = {
    ...
    "Questionable": [
        # Ambiguous items left for manual triage
...
def run_ai_batch_processing(items, is_batch_file=False):
    ...
    # Map output keys based on caller
    ...
    # Grab local pointers for thread safety
    with knowledge_base_lock:
            ...
            if final_id == "REQUIRES HUMAN" or final_match == "No good match":
                ...
                if ai_result and ai_result.get("matched_asset"):
                    ...
                    if conf == "low" or strict_match == "No good match":
                        ...
                        print(f"[{current}/{total_ai_items}] Ollama fixed it ({conf} confidence) -> {final_match}")
            else:
                with progress_lock:
                    processed_count[0] += 1

            return {k_row: row_id, k_match: final_match, k_id: final_id}
...
def prescrub_file():
    ...
    for row in records:
        ...
        processed_data.append({
            ...
            "Scrubbed Phrase": scrubbed, # Keep the original scrubbed for the UI
            "Asset Tag": asset_id,
