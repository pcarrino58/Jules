...
HIERARCHY_RULES = [
    ...
    {"group": "Fire Sprinkler", "parents": [r'\bsprinkler system', r'\bwet[- ]?pipe', r'\bdry[- ]?pipe', r'\bpre[- ]?action', r'\bdeluge', r'\bfire water storage tank', r'\bfire tank', r'\bstandpipe system', r'\bstandpipe\b'], "children": [r'\bsprinkler head', r'\bdrop\b', r'\bpressure reducing valve', r'\bprv\b', r'\bhose valve']},
...
def load_knowledge_base():
    ...
    for messy_term, clean_term in t_list_translations.items():
        if clean_term:
            ...
            # --- FIX: Changed from exact line match (r'^...$') to partial phrase match ---
            compiled_pat = re.compile(r'(?<![a-z0-9])' + re.escape(messy_term) + r'(?:s|es)?(?![a-z0-9])', re.IGNORECASE)
...
def prescrub_text(raw_text, use_whitelist=True, current_valid_words=None, current_stemmed_whitelist=None, current_pattern=None):
    ...
    # --- FIX: CORE SIGNATURE GENERALIZATION ---
    # Forces Python to memorize the base noun instead of the exact messy string
    words = clean.split()
    filtered = []
    noise_set = {
        "north", "south", "east", "west", "ne", "nw", "se", "sw", "n", "s", "e", "w",
        "up", "down", "upper", "lower", "left", "right",
        "central", "middle", "front", "back", "center", "interior", "exterior", "peripheral",
        "int", "ext", "internal", "external", "perm", "temp",
        "stage", "phase", "tier", "primary", "secondary",
        "level", "floor", "fl", "zone", "area", "room", "rm",
        "wing", "serving", "served", "roof", "mezz", "penthouse", "ph", "podium",
        "unit", "equip", "equipment", "system", "systems", "no", "num", "number",
        "conc", "concealed", "bldg", "building", "garage", "parkade", "p", "st",
        "base", "trk", "track", "mechanical", "mech",
        "picture", "clim", "nclim", "heated", "unheated", "steam", "n-heated", "old", "new", "located",
        "tower", "cu", "elec", "electrical", "vault", "bsmt", "basement", "hall", "corridor", "closet", "shaft", "lobby"
    }
    
    for w in words:
        if len(w) <= 1: continue # Strip stray single letters (A, B, C)
        if w.isdigit(): continue # Strip isolated numbers
        if w in noise_set: continue # Strip architectural/directional noise
        if any(c.isdigit() for c in w): continue # Strip alphanumeric tags (e.g., 'f15', '17th')
        filtered.append(w)
...
def run_ai_batch_processing(items, is_batch_file=False, skip_ollama=False):
    ...
    for row in items:
        ...
        # --- FIX 1: Allow Plurals in Generic Pump Fallback ---
        s_pad = f" {semantic_phrase} "
        if " pump " in s_pad or " pumps " in s_pad:
            ...
            has_specific = False
            for p in specific_pumps:
                if f" {p} " in s_pad or f" {p}s " in s_pad:
                    has_specific = True
                    break
    ...
    if ai_queue_phrases:
        ...
        def process_single_ai_group(i_queue):
            ...
            # --- FIX 2: Expand AI Dropdown Choices ---
        # Inside process_single_ai_group(i_queue):
            ...
            elif best_combined_score >= 0.35: # Lowered the floor slightly
                for m in unique_matches:
                    # Widened the score net from 0.15 to 0.20
                    if m[0] >= 0.35 and (best_combined_score - m[0] <= 0.20):
                        v = validate_and_format(m[1])
                        if v != "No good match" and v not in valid_options:
                            valid_options.append(v)
                            # Increased max dropdown choices from 6 to 10
                            if len(valid_options) >= 10: break

            if len(valid_options) > 1:
                final_match = "\n".join(valid_options[:10])
        ...
        # Cap max_workers to 2 to prevent RTX 5070 VRAM Queue issues
        with ThreadPoolExecutor(max_workers=2) as executor:
            futures = {executor.submit(worker, i): i for i in range(total_groups)}
            for future in as_completed(futures):
                results.extend(future.result())

    # STRICT 1-TO-1 ALIGNMENT RECOVERY: Sort mathematically before returning
    results.sort(key=lambda x: int(x[k_row]))
    ...
    with custom_rules_lock:
        for item in items:
            ...
            if signature and corrected_match:
                ...
                # Fallback print so you know exactly why a phrase was rejected
                print(f"[AI WARNING] Could not learn phrase. Signature generated: '{signature}'", flush=True)

    if learned_count > 0:
        recompile_custom_rules_global()
        print(f"\n==========================================", flush=True)
        print(f">>> [AI MEMORY] Successfully learned {learned_count} new override(s)! <<<", flush=True)
        print(f"==========================================\n", flush=True)
...
def prescrub_file():
    ...
    def process_prescrub(in_file, out_file):
        ...
        try:
            ...
            # Enforce strict index alignment based on source rows
            df_processed['Row'] = pd.to_numeric(df_processed['Row'])
            df_processed.sort_values('Row', inplace=True)
            df_processed['__has_boiler'] = df_processed['Scrubbed Phrase'].str.contains(r'\bboiler\b|\bblr\b', case=False, regex=True, na=False)
            
            unique_buildings = df_processed['Building'].unique()
            for bldg in unique_buildings:
                if not bldg or str(bldg).strip().lower() == 'nan': continue
                
                b_mask = df_processed['Building'] == bldg
                ambiguous_mask = b_mask & (df_processed['System Group'] == 'Ambiguous Fuel System')
                
                if ambiguous_mask.any():
                    has_fp = (b_mask & (df_processed['System Group'] == 'Fire Pump System') & (df_processed['Hierarchy'] == 'Parent')).any()
                    has_gen = (b_mask & (df_processed['System Group'] == 'Emergency Power') & (df_processed['Hierarchy'] == 'Parent')).any()
                    has_boiler = df_processed.loc[b_mask, '__has_boiler'].any()
                    ...
                    day_tank_indices = df_processed[ambiguous_mask][is_day_tank].index
                    other_indices = df_processed[ambiguous_mask][~is_day_tank].index
                ...
                for rule in HIERARCHY_RULES:
                    ...
                    group_mask = b_mask & (df_processed['System Group'] == group_name)
                    
                    if not group_mask.any(): continue
                        
                    p_count = (df_processed.loc[group_mask, 'Hierarchy'] == 'Parent').sum()
                    c_count = (df_processed.loc[group_mask, 'Hierarchy'] == 'Child').sum()
                    audit_msg = ""
                    ...
                    if audit_msg: 
                        df_processed.loc[df_processed[group_mask].index, 'Audit Flag'] = audit_msg

                for group_name in ["Life Safety / Fire", "Emergency Power", "Fire Pump System", "Fire Sprinkler"]:
                    group_mask = b_mask & (df_processed['System Group'] == group_name)
                    if not group_mask.any(): continue
                        
                    parents_idx = df_processed[(group_mask) & (df_processed['Hierarchy'] == 'Parent')].index
                    children_idx = df_processed[(group_mask) & (df_processed['Hierarchy'] == 'Child')].index
                    
                    p_count = len(parents_idx)
                    c_count = len(children_idx)
                    ...
                    if p_count > 0:
                        ...
                        if group_name == "Life Safety / Fire":
                            ...
                            diesel_mask = df_processed.loc[parents_idx, 'Scrubbed Phrase'].str.contains(r'diesel|engine', case=False, regex=True, na=False)
                            ...
                            dry_mask = df_processed.loc[parents_idx, 'Scrubbed Phrase'].str.contains(r'dry', case=False, regex=True, na=False)
                        ...
                        df_processed.loc[parents_idx, 'Yearly Effort (Hrs)'] = round(total_effort / p_count, 2)
                        df_processed.loc[parents_idx, 'Yearly Material ($)'] = round(total_mat / p_count, 2)
                        df_processed.loc[children_idx, 'Yearly Effort (Hrs)'] = 0.0
                        df_processed.loc[children_idx, 'Yearly Material ($)'] = 0.0
                    else:
                        if group_name == "Life Safety / Fire":
                            df_processed.loc[children_idx, 'Yearly Effort (Hrs)'] = 0.05
                        elif group_name == "Fire Pump System":
                            df_processed.loc[children_idx, 'Yearly Effort (Hrs)'] = 2.78
                        elif group_name == "Emergency Power":
                            df_processed.loc[children_idx, 'Yearly Effort (Hrs)'] = 2.0
            ...
            for bldg in unique_buildings:
                ...
                b_mask = df_processed['Building'] == bldg
                fire_parents = df_processed[b_mask & (df_processed['NFPA Group'].str.contains('Group 1|Group 2', na=False)) & (df_processed['Hierarchy'] == 'Parent')]
                has_fire_parent = len(fire_parents) > 0
                has_backflow = df_processed.loc[b_mask, 'Scrubbed Phrase'].str.contains(r'\bbackflow\b', case=False, regex=True, na=False).any()
                ...
                if building_alerts:
                    first_idx = df_processed[b_mask].index[0]
