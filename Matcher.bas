...
Public Sub RunAIBulkCleanup()
    ...
    ' 1. GRAB EVERYTHING IN COLUMN A (UTF-8 SAFE WRITER) IN BULK
    Dim streamOut As Object
    Set streamOut = CreateObject("ADODB.Stream")
    streamOut.Type = 2 ' Text stream
    streamOut.Charset = "utf-8"
    ...
    ' --- HIGH SPEED ARRAY READ (Eliminates 11,000 sheet pings) ---
    ...
    For i = 2 To lastRow
        If Not IsError(arrIn(i, 1)) Then
            ...
            If Len(Trim(inputPhrase)) > 0 Then
                ...
                ' Scrub tabs, line feeds, and carriage returns so it stays on one line
    ...
    streamOut.SaveToFile inputFile, 2 ' 2 = SaveCreateOverWrite
    streamOut.Close
    
    If targetCount = 0 Then
        MsgBox "No assets found in Column A.", vbInformation
        GoTo Cleanup
    End If
    
    ' 2. PING PYTHON TO PROCESS THE ENTIRE SHEET
    Application.StatusBar = "? Python taking command: Processing " & targetCount & " assets..."
    
    ' --- DYNAMIC SPLASH SCREEN ---
    Dim splash As Shape
    Set splash = ws.Shapes.AddShape(msoShapeRectangle, 200, 150, 400, 100)
    splash.Name = "AISplashScreen"
    splash.TextFrame.Characters.text = "?? AI is processing " & targetCount & " assets." & vbCrLf & "Please wait..."
    splash.TextFrame.HorizontalAlignment = xlHAlignCenter
    splash.TextFrame.VerticalAlignment = xlVAlignCenter
    splash.TextFrame.Characters.Font.Size = 18
    splash.TextFrame.Characters.Font.Bold = True
    splash.Fill.ForeColor.RGB = RGB(255, 204, 0)
    splash.Line.ForeColor.RGB = RGB(0, 0, 0)
    splash.Line.weight = 2
    
    ' Force Excel to draw the box right before we freeze the screen
    Application.ScreenUpdating = True
    DoEvents
    Application.ScreenUpdating = False
    ' ----------------------------------
    ...
    If Err.Number <> 0 Then
        MsgBox "Failed to connect to Python server. Is the script running?", vbExclamation
        GoTo Cleanup
    End If
    On Error GoTo 0
    
    ' 3. BLIND PASTE THE RESULTS
    If http.Status = 200 Then
        If fso.FileExists(outputFile) Then
            Set tsOut = fso.OpenTextFile(outputFile, 1)
            Dim lineStr As String, parts() As String
            Dim rRow As Long, rMatch As String, rId As String
            
            ' --- HIGH SPEED ARRAY WRITE ---
            ...
            Dim requiresHuman As Object: Set requiresHuman = CreateObject("Scripting.Dictionary") '<-- NEW: Track human review rows
            
            If Not tsOut.AtEndOfStream Then lineStr = tsOut.ReadLine
            
            Do While Not tsOut.AtEndOfStream
                lineStr = tsOut.ReadLine
                parts = Split(lineStr, vbTab)
                ...
                If UBound(parts) >= 2 Then
                    ...
                    If rMatch <> "UNKNOWN" And rMatch <> "" And rMatch <> "SKIP" Then
                        ...
                        Else
                            ...
                            ' --- NEW: Flag for Yellow Paint if it requires human attention ---
                            If rId = "REQUIRES HUMAN" Or rMatch = "No good match" Then
                                requiresHuman.Add CStr(rRow), True
                            End If
                        End If
                    End If
                End If
            ...
            Application.StatusBar = "? Dropping Data into Excel..."
            DoEvents
            
            ' Drop the massive array into the sheet instantly
            ws.Range("B1:C" & lastRow).Value2 = sheetData
            
            ' Power-wash old validation and pink paint
            ws.Range("B2:B" & lastRow).Validation.Delete
            ws.Range("B2:C" & lastRow).Interior.Color = RGB(252, 186, 186)
            
            ' 5. APPLY DROPDOWNS & COLORS BATCHED
            Application.StatusBar = "? Building Dropdowns & Formatting..."
            DoEvents
...
Cleanup:
    ' Wipe the temporary flat files off the hard drive
    On Error Resume Next
    ...
    ws.Shapes("AISplashScreen").Delete
    On Error GoTo 0
    ...
    ' --- FORCE WINDOW TO FRONT AND SHOW SYSTEM MODAL POPUP ---
    AppActivate Application.Caption
    MsgBox "? Master Run Complete!" & vbCrLf & vbCrLf & "Evaluated " & targetCount & " assets purely through AI.", vbInformation + vbSystemModal, "AI Hand-off Complete"
End Sub
