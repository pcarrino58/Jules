...
Public Sub PropagateLearnedChoice(ByVal ws As Worksheet, ByVal sourceRow As Long, ByVal targetSig As String, ByVal targetNorm As String, ByVal chosenOut As String)
    ...
    ' Ensure dictionaries are alive to build the core asset names
    If mAliasDict Is Nothing Then Call EnsureDictionaries
    ...
    targetCore = GetCoreEquipmentName(NormalizeAndAlias(sourceRaw, mAliasDict))
        ...
        If r <> sourceRow Then
            If Not IsError(arrData(r, 1)) Then
                ...
                If Len(Trim$(raw)) > 0 Then
                    ...
                    ' Rule 1: Exact text match (ignoring spaces/caps)
                    If GetStandardKey(raw) = GetStandardKey(sourceRaw) Then
                        isMatch = True
                    Else
                        ' Rule 2: Core Equipment Match (e.g. "sprinkler dry" = "sprinkler dry")
                        Dim rowCore As String
                        rowCore = GetCoreEquipmentName(NormalizeAndAlias(raw, mAliasDict))
                        
                        If Len(targetCore) > 0 And rowCore = targetCore Then
                            isMatch = True
                        End If
                    End If
...
Public Function IsValueFromDropdown(ws As Worksheet, rowNum As Long, chosenVal As String) As Boolean
    ...
    ' 2. Check Column C (Current Status) BEFORE it gets overwritten
    ' If Python or VBA flagged this row as having multiple options, block permanent learning!
    Dim currentStatus As String
    currentStatus = CStr(ws.Cells(rowNum, "C").Value2)
    
    If InStr(1, currentStatus, "Multiple Options", vbTextCompare) > 0 Or _
       InStr(1, currentStatus, "Multiple Rules", vbTextCompare) > 0 Then
        IsValueFromDropdown = True ' Returns True to SKIP saving permanently
        Exit Function
    End If

    ' 3. Fallback: Re-run the VBA matcher silently just in case Column C was cleared
    ...
    ' IF it was an AI Guess, AI Vector, or Manual Entry, we return FALSE
    ' so that the system is FORCED to learn it and queue it for Python.
    IsValueFromDropdown = False
End Function
...
Private Sub GetHierarchyInfo(ByVal assetType As String, ByRef outGroup As String, ByRef outRole As String)
    ...
    ' --- FIRE SPRINKLER ---
    If InStr(s, " sprinkler head") > 0 Or InStr(s, " drop") > 0 Then
        outGroup = "Fire Sprinkler": outRole = "Child": Exit Sub
    End If
    If InStr(s, " sprinkler system") > 0 Or InStr(s, " wet pipe") > 0 Or InStr(s, " dry pipe") > 0 Or InStr(s, " pre-action") > 0 Or InStr(s, " deluge") > 0 Then
...
Private Function GetCoreEquipmentName(ByVal text As String) As String
    ...
    For i = LBound(parts) To UBound(parts)
        ...
        ' Strip out single letters (A, B, C)
        If Len(w) <= 1 Then GoTo SkipWord
        
        ' Aggressively strip ANY word that contains a number
        If IsNumeric(w) Or HasAnyDigit(w) Or IsTagLike(w) Then GoTo SkipWord
        
        ' Strip out common facility descriptors and directions
        Select Case w
            Case "north", "south", "east", "west", "ne", "nw", "se", "sw", _
                 "up", "down", "upper", "lower", "left", "right", _
                 "central", "middle", "front", "back", "center", _
                 ...
                 "conc", "concealed", "bldg", "building", "garage", "parkade", "p", "st"
                ' Skip it
            Case Else
                out = out & w & " "
        End Select
