...
Public Sub GetBestMatchForInput(ByVal rawInput As String, ByRef outVal As String, ByRef confVal As String)
    ' =========================================================
    ' NEW: SAFETY SHIELD TO PREVENT ERROR 91
    ' =========================================================
    ...
    ' -------------------------------------------------------------------------
    ' PRIORITY 5 & 6: SIGNATURE MATCHES
    ' -------------------------------------------------------------------------
...
' --- NEW HELPER FUNCTION TO CATCH "PACKAGE A" vs "PACKAGE B" (OMNI-DIRECTIONAL) ---
Private Function IsSameEquipmentFamily(ByVal str1 As String, ByVal str2 As String) As Boolean
    ...
    Dim diff1 As String, diff2 As String
    ...
    ' If EXACTLY ONE word differs, and it is a short designation (A, B, 1, 12, C1)
    ' regardless of where it is located in the string, they are the same family!
    If diffCount = 1 Then
        If Len(diff1) <= 3 And Len(diff2) <= 3 Then
...
Public Sub RunAISniper(ws As Worksheet, rowNum As Long, inputPhrase As String)
    ...
    Dim url As String: url = AI_API_BASE_URL & "/batch_lookup"
...
' DECISION ENGINE
    If matchStr <> "" And matchStr <> "UNKNOWN" And matchStr <> "No good match" Then
        ...
        If confStr = "AI_HYBRID_MATCH" Then
            ...
            ' --- UPDATED AI GUESSES DROPDOWN LOGIC ---
...
' ==========================================================
' THE DUMB PIPE ARCHITECTURE: RunAIBulkCleanup (HIGH SPEED)
' ==========================================================
Public Sub RunAIBulkCleanup()
    ...
    ' 1. FAST BULK WRITE TO TSV (WITH SAFE SHIELD)
    Dim streamOut As Object
    Set streamOut = CreateObject("ADODB.Stream")
    ...
    ' Added Manufacturer to headers
    streamOut.WriteText "Row" & vbTab & "Phrase" & vbTab & "Manufacturer" & vbTab & "Model" & vbTab & "Serial" & vbCrLf
    
    Dim arrIn As Variant, arrStatus As Variant
    Dim arrMfg As Variant, arrModel As Variant, arrSerial As Variant
    
    arrIn = ws.Range("A1:A" & lastRow).Value2
    arrStatus = ws.Range("C1:C" & lastRow).Value2
    arrMfg = ws.Range("AP1:AP" & lastRow).Value2   '<-- Manufacturer from AP
    ...
    For i = 2 To lastRow
        If Not IsError(arrIn(i, 1)) Then
            ...
            Dim needsAI As Boolean
            needsAI = False
            
            ' Only trigger AI for unresolved or blank cells
            ...
            If needsAI And Len(Trim$(inputPhrase)) > 0 Then
                ...
                ' Extract and clean Model/Serial data
                Dim mfgStr As String, modelStr As String, serialStr As String
                ...
                streamOut.WriteText i & vbTab & inputPhrase & vbTab & mfgStr & vbTab & modelStr & vbTab & serialStr & vbCrLf
            End If
        End If
    Next i
    ...
    ' 2. PING PYTHON SERVER
    Dim http As Object
    Set http = CreateObject("MSXML2.ServerXMLHTTP.6.0")
    If http Is Nothing Then Set http = CreateObject("MSXML2.XMLHTTP")
    
    ' Bumped connect/send up to 60 seconds, receive stays at 1 hour
    http.setTimeouts 60000, 60000, 60000, 3600000
    ...
    On Error Resume Next
    Err.Clear
    http.Open "POST", AI_API_BASE_URL & "/batch_file", False
    http.setRequestHeader "Content-Type", "application/json"
    http.send jsonPayload
    
    ' --- NEW DIAGNOSTIC ERROR HANDLER ---
    ...
    ' 3. FAST BULK READ FROM TSV (In-Memory ADODB)
    If fso.FileExists(outputFile) Then
                ...
                If UBound(parts) >= 2 Then
                    ...
                    If rMatch <> "UNKNOWN" And rMatch <> "" And rMatch <> "SKIP" Then
                        ...
                        ElseIf InStr(rMatch, vbLf) > 0 Then
                            ...
                            yellowRows(rRow) = True
                        Else
                            ' Drop the exact answer into the cell
                            sheetData(rRow, 1) = rMatch
                            sheetData(rRow, 2) = rId
                            
                            ' Route to the correct color bucket
        ...
        ' Write entire dataset back in one operation
...
Public Sub FlushBatchLearningsToPython()
    ...
    On Error Resume Next
    ' Extended timeouts to allow Python to process large batches safely
...
Public Sub RunDataPreScrub()
    ...
    ' Target your main Helper Sheet
    On Error Resume Next
    Set wsSource = ThisWorkbook.Worksheets("Helper Sheet")
    On Error GoTo 0
    If wsSource Is Nothing Then
        MsgBox "Cannot find 'Helper Sheet'.", vbCritical
        Exit Sub
    End If
    ...
    streamOut.Charset = "utf-8"
    streamOut.Open
    streamOut.WriteText "Row" & vbTab & "Building" & vbTab & "Phrase" & vbTab & "Manufacturer" & vbTab & "Model" & vbTab & "Serial" & vbCrLf
    
    Dim arrIn As Variant, arrBldg As Variant, arrMfg As Variant, arrModel As Variant, arrSerial As Variant
    ...
    ' --- NEW LOGIC: WRITE EVERY ROW (EVEN BLANKS) TO PRESERVE 1-TO-1 ALIGNMENT ---
    For i = 2 To lastRow
        inputPhrase = ""
        buildingStr = ""
        mfgStr = ""
        ...
        streamOut.WriteText i & vbTab & buildingStr & vbTab & inputPhrase & vbTab & mfgStr & vbTab & modelStr & vbTab & serialStr & vbCrLf
    Next i
    
    streamOut.SaveToFile inputFile, 2
    streamOut.Close
    
    ' 2. PING PYTHON PRE-SCRUB ENDPOINT
    Dim http As Object
    Set http = CreateObject("MSXML2.ServerXMLHTTP.6.0")
    If http Is Nothing Then Set http = CreateObject("MSXML2.XMLHTTP")
    ...
    ' 3. PREPARE THE SCRUBBED INPUTS SHEET
    On Error Resume Next
    ...
    wsScrub.Cells.Clear
    wsScrub.Range("A1").Value = "Row"
    wsScrub.Range("B1").Value = "Building"
    wsScrub.Range("C1").Value = "Original Phrase"
    wsScrub.Range("D1").Value = "Scrubbed Phrase"
    wsScrub.Range("E1").Value = "LLM Model Identity"
    wsScrub.Range("F1").Value = "Technical Specs"
    wsScrub.Range("G1").Value = "Consumables"
    wsScrub.Range("H1").Value = "Lifecycle Data"
    wsScrub.Range("I1").Value = "Asset Tag"
    wsScrub.Range("J1").Value = "Location"
    wsScrub.Range("K1").Value = "System Group"
    wsScrub.Range("L1").Value = "Hierarchy"
    wsScrub.Range("M1").Value = "Audit Flag"
    wsScrub.Range("N1").Value = "PM Suitability"
    wsScrub.Range("O1").Value = "Yearly Effort (Hrs)"
    wsScrub.Range("P1").Value = "Yearly Material ($)"
    wsScrub.Range("Q1").Value = "NFPA Group"
    ...
    If fso.FileExists(outputFile) Then
        ...
        For i = 1 To UBound(lines)
            If Len(lines(i)) > 0 Then
                ...
                If UBound(parts) >= 16 Then
                    writeData(writeCount, 1) = parts(0)
                    writeData(writeCount, 2) = parts(1)
                    writeData(writeCount, 3) = parts(2)
                    writeData(writeCount, 4) = parts(3)
                    writeData(writeCount, 5) = parts(4)
                    writeData(writeCount, 6) = parts(5)
                    writeData(writeCount, 7) = parts(6)
                    writeData(writeCount, 8) = parts(7)
                    writeData(writeCount, 9) = parts(8)
                    writeData(writeCount, 10) = parts(9)
                    writeData(writeCount, 11) = parts(10)
                    writeData(writeCount, 12) = parts(11)
                    writeData(writeCount, 13) = parts(12)
                    writeData(writeCount, 14) = parts(13)
                    writeData(writeCount, 15) = parts(14)
                    writeData(writeCount, 16) = parts(15)
                    writeData(writeCount, 17) = parts(16)
        ...
        If writeCount > 1 Then
            ...
            Dim lastScrub As Long
            lastScrub = writeCount
    ...
    ' STATIC XLOOKUP REPLACEMENT (Filtered to prevent warnings in Helper Sheet Col F)
    Dim lastScrubRow As Long
    lastScrubRow = wsScrub.Cells(wsScrub.Rows.count, "A").End(xlUp).Row
    
    If lastScrubRow >= 2 Then
        Dim scrubArr As Variant
        scrubArr = wsScrub.Range("A2:N" & lastScrubRow).Value2
        
        Dim pmArr As Variant
        ReDim pmArr(1 To lastRow - 1, 1 To 1)
