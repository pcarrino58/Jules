...
Public Sub SyncLookupsWithExternal(ByVal wb As Workbook, ByVal sheetName As String)
    ...
    If lastRow >= 2 Then
        For r = 2 To lastRow
            lineStr = Trim$(CStr(ws.Cells(r, "A").Value))
            If Len(lineStr) > 0 Then
...
' 1. FAST BULK WRITE TO TSV (WITH SAFE SHIELD)
    ...
    http.setTimeouts 5000, 5000, 10000, 300000
    ...
    If fso.FileExists(outputFile) Then
        ...
        For Each k In aiGuesses.keys
            ApplyDropdownToCell ws.Cells(CLng(k), "B"), aiGuesses(k) & ",Reject AI Guess"
        Next k
...
' Apply Yellow Highlighting in a Single Direct Pass
        For Each k In yellowRows.keys
            ws.Range("B" & k & ":C" & k).Interior.Color = RGB(255, 255, 0)
        Next k
...
Public Function IsValueFromDropdown(ws As Worksheet, rowNum As Long, chosenVal As String) As Boolean
    ...
    ' Did the matcher originally produce a Multiple Rules list?
    ...
    ' Did the matcher originally produce an AI Hybrid Guess?
    ...
    ' NEW: Did the matcher originally produce an AI Suggested List?
    If confVal = "AI_SUGGESTED_LIST" Then
        raw = Replace(outVal, vbCrLf, vbLf)
        raw = Replace(raw, vbCr, vbLf)
        parts = Split(raw, vbLf)
    ...
    ' NEW: Did the AI provide a smart vector match or expert rule?
    If confVal = "AI_SMART_VECTOR" Or confVal = "EXPERT_RULE" Then
        If LCase$(Trim$(outVal)) = cleanChosen Then
            IsValueFromDropdown = True
            Exit Function
        End If
    ...
    If confVal = "Exact Match" Or confVal = "Rule Hit" Or confVal = "Pattern Match" Then
        If LCase$(Trim$(outVal)) = cleanChosen Then
            IsValueFromDropdown = True
            Exit Function
        End If
    ...
    ' If it wasn't in any generated list or rule, they must have manually pasted it! OK TO LEARN!
    IsValueFromDropdown = False
End Function

' ==========================================================
' NEW: Sends user corrections directly to the Python AI memory
' ==========================================================
Private Sub TeachPythonAI(ByVal messyPhrase As String, ByVal cleanMatch As String)
    ...
    http.Open "POST", AI_API_BASE_URL & "/learn", False
    http.setRequestHeader "Content-Type", "application/json"
    http.send jsonPayload
    On Error GoTo 0
End Sub
...
Public Sub RunDataPreScrub()
    ...
    ' 1. FAST BULK WRITE (Extracting Column A and Column D)
    Dim streamOut As Object
    Set streamOut = CreateObject("ADODB.Stream")
    streamOut.Type = 2
    streamOut.Charset = "utf-8"
    ...
    For i = 2 To lastRow
        If Not IsError(arrIn(i, 1)) Then
            ...
            If Len(Trim$(inputPhrase)) > 0 Then
                ...
                ' Handle empty Building fields gracefully
                If Not IsError(arrBldg(i, 1)) Then
                    buildingStr = CStr(arrBldg(i, 1))
                Else
                    buildingStr = ""
                End If
    ...
    wsScrub.Range("E1").Value = "System Group"
    wsScrub.Range("F1").Value = "Hierarchy"
    wsScrub.Range("G1").Value = "Audit Flag"
    wsScrub.Range("A1:G1").Font.Bold = True
    wsScrub.Range("A1:G1").Interior.Color = RGB(220, 230, 241)
    ...
    If fso.FileExists(outputFile) Then
        ...
        ReDim writeData(1 To UBound(lines), 1 To 7)
        ...
        For i = 1 To UBound(lines)
            If Len(lines(i)) > 0 Then
                parts = Split(lines(i), vbTab)
                ' Unpack the 7 columns generated by Python
                If UBound(parts) >= 6 Then
                    ...
                    writeData(writeCount, 5) = parts(4) ' System Group
                    writeData(writeCount, 6) = parts(5) ' Hierarchy
                    writeData(writeCount, 7) = parts(6) ' Audit Flag
        ...
        If writeCount > 1 Then
            wsScrub.Range("A2").Resize(writeCount - 1, 7).Value2 = writeData
            wsScrub.Columns("A:G").AutoFit
            
            ' Add basic color formatting for Audit Flags in Column G
            Dim lastScrub As Long
            lastScrub = writeCount
            
            ' Color warnings yellow
            wsScrub.Range("G2:G" & lastScrub).FormatConditions.Add Type:=xlTextString, String:="??", TextOperator:=xlContains
            wsScrub.Range("G2:G" & lastScrub).FormatConditions(1).Interior.Color = RGB(255, 235, 156)
            wsScrub.Range("G2:G" & lastScrub).FormatConditions(1).Font.Color = RGB(156, 101, 0)
            
            ' Color valid states green
            wsScrub.Range("G2:G" & lastScrub).FormatConditions.Add Type:=xlTextString, String:="Matched", TextOperator:=xlContains
            wsScrub.Range("G2:G" & lastScrub).FormatConditions(2).Interior.Color = RGB(198, 239, 206)
            wsScrub.Range("G2:G" & lastScrub).FormatConditions(2).Font.Color = RGB(0, 97, 0)
            
            wsScrub.Range("G2:G" & lastScrub).FormatConditions.Add Type:=xlTextString, String:="Valid", TextOperator:=xlContains
            wsScrub.Range("G2:G" & lastScrub).FormatConditions(3).Interior.Color = RGB(198, 239, 206)
            wsScrub.Range("G2:G" & lastScrub).FormatConditions(3).Font.Color = RGB(0, 97, 0)
        End If
    End If
...
' ==========================================================
' PARENT / CHILD PM TRIGGER LOGIC WITH MISSING PARENT AUDIT
' ==========================================================
Public Sub CalculatePMRules(ByVal ws As Worksheet, Optional ByVal showPopup As Boolean = False)
    ...
    ' --------------------------------------------------------
    ' LOAD MANUAL OVERRIDES FROM TRIAGE BOARD
    ' --------------------------------------------------------
    ...
    For i = 2 To lastRow
        If Not IsError(arrAsset(i, 1)) And Not IsError(arrBldg(i, 1)) Then
            ...
            If Len(asset) > 0 And Len(bldg) > 0 Then
                ...
                ' Apply Triage Override if it exists
    ...
    Call BuildLiveDashboard(ws, pCountDict, cCountDict)
...
Private Sub GetHierarchyInfo(ByVal assetType As String, ByRef outGroup As String, ByRef outRole As String)
    ...
    If InStr(s, " day tank") > 0 Or InStr(s, " fuel polishing") > 0 Or InStr(s, " fuel tank") > 0 Or InStr(s, " fuel oil tank") > 0 Or InStr(s, " transfer pump") > 0 Then
        ' Exception: If it explicitly says "fire pump", let it stay Fire Pump System (handled above)
...
' ==========================================================
' SCRUBBED INPUTS: MISSING PARENT DASHBOARD
' ==========================================================
Public Sub BuildMissingParentSummary()
    ...
    ' 1. Clear any old summary in columns I:K
    ws.Range("I:K").Clear
    ...
    ' 2. Loop through the Python output and aggregate missing parents
    For i = 2 To lastRow
        bldg = Trim(CStr(ws.Cells(i, "B").Value2))
        flag = Trim(CStr(ws.Cells(i, "G").Value2))
        
        ' Only grab rows that contain the warning emoji / missing text
    ...
    ' 3. Build the Dashboard Headers
    ws.Cells(1, "I").Value2 = "BUILDING"
    ws.Cells(1, "J").Value2 = "MISSING PARENT ALERT"
    ws.Cells(1, "K").Value2 = "CHILDREN IMPACTED"
    
    With ws.Range("I1:K1")
        .Font.Bold = True
        .Interior.Color = RGB(0, 112, 192) ' Standard Blue Header
        .Font.Color = vbWhite
    End With
    
    ' 4. Print the Dashboard Results
    Dim printRow As Long
    printRow = 2
    
    If missingDict.count = 0 Then
        ws.Cells(printRow, "I").Value2 = "All Buildings"
        ws.Cells(printRow, "J").Value2 = "All Children Have Parents!"
        ws.Cells(printRow, "K").Value2 = "-"
        ws.Range("I2:K2").Interior.Color = RGB(201, 231, 167) ' All Clear Green
    Else
        Dim k As Variant
        Dim parts() As String
        For Each k In missingDict.keys
            parts = Split(CStr(k), "|")
            ws.Cells(printRow, "I").Value2 = parts(0)
            ws.Cells(printRow, "J").Value2 = parts(1)
            ws.Cells(printRow, "K").Value2 = missingDict(k)
            
            ' Color the rows in the standard alert pink
            ws.Range("I" & printRow & ":K" & printRow).Interior.Color = RGB(252, 186, 186)
            printRow = printRow + 1
        Next k
    End If
    
    ' 5. Auto-size the columns so it looks clean
...
Private Sub BuildLiveDashboard(ByVal ws As Worksheet, ByVal pCount As Object, ByVal cCount As Object)
    ' 1. Clear old dashboard in I:K
    ws.Range("I:K").Clear
    
    Dim k As Variant, bName As String, gName As String, parts() As String
    ...
    For Each k In pCount.keys: allKeys(k) = True: Next k
    For Each k In cCount.keys: allKeys(k) = True: Next k
    
    If allKeys.count = 0 Then Exit Sub
    
    Dim results() As Variant
    ReDim results(1 To allKeys.count, 1 To 3)
    Dim rowIdx As Long: rowIdx = 0
    
' 2. Calculate missing components
    For Each k In allKeys.keys
        parts = Split(CStr(k), "|")
        bName = parts(0)
        gName = parts(1)
        If pCount.Exists(k) Then p = pCount(k) Else p = 0
        If cCount.Exists(k) Then c = cCount(k) Else c = 0
        auditMsg = ""
        
        If gName = "Split HVAC" Then
            If p > 0 And c = 0 Then
                auditMsg = "?? Split HVAC: Missing Child (e.g., Fan Coil, Indoor Unit)"
            ElseIf c > 0 And p = 0 Then
                auditMsg = "?? Split HVAC: Missing Parent (e.g., Condenser, Outdoor Unit)"
            End If
        Else
            If c > 0 And p = 0 Then
                Dim pLabel As String
                If gName = "Life Safety / Fire" Then
                    pLabel = "Parent (e.g., Main FACP, Fire Panel)"
                ElseIf gName = "Emergency Power" Then
                    pLabel = "Parent (e.g., Emergency Generator)"
                ElseIf gName = "Fire Pump System" Then
                    pLabel = "Parent (e.g., Main Fire Pump)"
                ElseIf gName = "Vertical Transport" Then
                    pLabel = "Parent (e.g., Elevator Cab, Escalator)"
                ElseIf gName = "Fire Sprinkler" Then
                    pLabel = "Parent (e.g., Sprinkler Riser System)"
                Else
                    pLabel = "Parent Asset"
                End If
                auditMsg = "?? " & gName & ": Missing " & pLabel
            End If
        End If
        
        ' 3. Add to display array
        If auditMsg <> "" Then
            rowIdx = rowIdx + 1
            results(rowIdx, 1) = bName
            results(rowIdx, 2) = auditMsg
            If InStr(auditMsg, "Missing Child") > 0 Then results(rowIdx, 3) = p Else results(rowIdx, 3) = c
        End If
    Next k
    
' 4. Print Headers and Format
    ws.Cells(1, "I").Value2 = "BUILDING"
    ws.Cells(1, "J").Value2 = "MISSING PARENT ALERT"
    ws.Cells(1, "K").Value2 = "CHILDREN IMPACTED"
    
    ' THIS IS THE NEW LINE TO PREVENT THE PIVOT CRASH:
    ws.Cells(1, "L").Value2 = "Spacer"
    ...
    End With
    If rowIdx > 0 Then
        ws.Range("I2").Resize(rowIdx, 3).Value2 = results
        ws.Range("I2").Resize(rowIdx, 3).Interior.Color = RGB(252, 186, 186)
    Else
        ws.Cells(2, "I").Value2 = "All Buildings"
        ws.Cells(2, "J").Value2 = "All Children Have Parents!"
        ws.Cells(2, "K").Value2 = "-"
        ws.Range("I2:K2").Interior.Color = RGB(201, 231, 167)
    End If
    ws.Columns("I:K").AutoFit
End Sub

Public Sub FixHelperHeaders()
    Dim ws As Worksheet
    Set ws = ActiveSheet
    ...
    ' Scans the first 50 columns of Row 1 for blank spaces
    For i = 1 To 50
        If Trim(ws.Cells(1, i).Value2) = "" Then
            ws.Cells(1, i).Value2 = "Spacer_" & i
            ws.Cells(1, i).Interior.Color = RGB(255, 255, 0) ' Highlights it yellow so you see it
            fixCount = fixCount + 1
        End If
    Next i
    
    MsgBox "Found and fixed " & fixCount & " missing headers!", vbInformation, "Header Fix Complete"
End Sub
