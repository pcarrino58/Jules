...
Private mBatchLearnDict As Object
...
Private Const AI_API_BASE_URL As String = "http://127.0.0.1:5000"
...
Public Sub EmergencyClearSheet()
    ' 1. Route any errors directly to the cleanup block
    ...
    ' Wipes everything but the headers across all active columns (A through F)
    Worksheets("Helper Sheet").Range("A2:F100000").ClearContents
    Worksheets("Helper Sheet").Range("B2:F100000").Validation.Delete
    Worksheets("Helper Sheet").Range("B2:F100000").Interior.Color = xlNone
    
SafeExit:
    ' 3. UNFREEZE the application
    Application.EnableEvents = True
    Application.ScreenUpdating = True
    
    ' 4. Handle any errors
    If Err.Number <> 0 Then
        MsgBox "Failed to clear the sheet. Ensure 'Helper Sheet' exists and is unprotected." & vbCrLf & "Error: " & Err.Description, vbCritical
    End If
End Sub
...
' Expand target range to include columns D, E, and F
    Dim targetRange As Range
    Set targetRange = wsInput.Range("A" & minRow & ":F" & maxRow)
    ...
    For i = 1 To UBound(dataArr, 1)
        ...
        If rowsDict.Exists(actualRow) Then
            If Not IsError(dataArr(i, 1)) Then
                ...
                Else
                    ' NEW: If Column A is deleted, wipe the rest of the row's data
                    dataArr(i, 2) = ""
                    dataArr(i, 3) = ""
                    dataArr(i, 4) = "" ' Building
                    dataArr(i, 5) = "" ' PM Trigger
                    dataArr(i, 6) = "" ' PM Feasibility
                End If
            Else
...
Public Sub RecordLearnedOverride(ByVal rowNum As Long, Optional ByVal isBulkPaste As Boolean = False)
...
' --- ONLY SAVE TO DISK IF NOT BULK PASTING ---
        If Not isBulkPaste Then
            AppendToExternalFile normalized, chosen, sig
        End If

        ' --- NEW: QUEUE CORRECTION FOR PYTHON AI BATCH ---
        If mBatchLearnDict Is Nothing Then Set mBatchLearnDict = CreateObject("Scripting.Dictionary")
        mBatchLearnDict(rawInput) = chosen
        
        ' If pasting a single cell, flush the queue immediately
        If Not isBulkPaste Then
            Call FlushBatchLearningsToPython
        End If
    End If

    ' --- ALWAYS TRIGGER AUTO-PROPAGATION (UNLESS BULK PASTING) ---
    If Not isBulkPaste Then
        Call PropagateLearnedChoice(wsInput, rowNum, sig, normalized, chosen)
        
        ' --- NEW: Instantly recalculate PM Rules so Columns E & F update ---
        ' Call CalculatePMRules(wsInput)
    End If
End Sub
...
Public Function IsValidOutputList(ByVal outP As String) As Boolean
    ...
    Dim k As Variant, rParts() As String, rIdx As Long
    ...
    For i = LBound(parts) To UBound(parts)
        ...
        If Len(s) > 0 Then
            ...
            ' 1. Check Master Uniformat Sheet
            If mLookupPhraseSet.Exists(s) Then found = True
            
            ' 2. Check Rules Sheet Outputs
            If Not found And Not mForcedOutputDict Is Nothing Then
                For Each k In mForcedOutputDict.keys
                    rParts = Split(CStr(mForcedOutputDict(k)), vbLf)
                    For rIdx = LBound(rParts) To UBound(rParts)
                        If GetStandardKey(rParts(rIdx)) = s Then
                            found = True
                            Exit For
                        End If
                    Next rIdx
            ...
            ' 3. Check Learned Mappings Outputs
            If Not found And Not mLearnedExact Is Nothing Then
                For Each k In mLearnedExact.keys
                    If GetStandardKey(CStr(mLearnedExact(k))) = s Then
                        found = True
                        Exit For
                    End If
                Next k
            ...
            ' If it exists nowhere in the architecture, reject it
            If Not found Then
...
Public Sub GetBestMatchForInput(ByVal rawInput As String, ByRef outVal As String, ByRef confVal As String)
    If mStopWords Is Nothing Or mSignatureDict Is Nothing Or mAliasDict Is Nothing Then
    ...
    ' --- FIX: Allow Plurals in Generic Pump Fallback ---
    If InStr(1, sPad, " pump ", vbTextCompare) > 0 Or InStr(1, sPad, " pumps ", vbTextCompare) > 0 Then
        ...
        pumpTypes = Array("sump", "submersible", "fuel", "condensate", "hydraulic", _
                          ...
                          "centrifugal", "circ", "circulation", "dosing", "chem", _
                          "heat", "glycol")

        isSpecificPump = False
        For Each pType In pumpTypes
            ' Check for singular OR plural specific pumps
            If InStr(1, sPad, " " & pType & " ", vbTextCompare) > 0 Or InStr(1, sPad, " " & pType & "s ", vbTextCompare) > 0 Then
                isSpecificPump = True
                Exit For
            End If
        Next pType
    ...
    ' Regex Double-Processing Fix Applied Here
    inputSig = GetCanonicalSignature(normalizedInput, True)
    ...
    inputSig = GetVocabSignature(normalizedInput, True)
    If Len(inputSig) > 0 Then
        If mSignatureDict.Exists(inputSig) Then
            outVal = mSignatureDict(inputSig)
...
' =========================================================
    ' FINAL OUTPUT GENERATION (UPDATED FOR AI ROUTING)
    ' =========================================================
...
Private Sub InitializeMatcher(wsLookup As Worksheet)
    FlushMemory '<--- Purge old memory before loading fresh data
    MIN_MATCH_THRESHOLD = 0.5
    mLearnedLoaded = False ' Force reload of learned data
    
    ' The missing engine rebuild commands:
    If wsLookup Is Nothing Then Exit Sub
    EnsureDictionaries
...
Private Function IsNonAssetInput(ByVal sNorm As String) As Boolean
    ...
    badWords = Array(" report ", " reports ", " survey ", " surveys ", " audit ", " audits ", _
                     ...
                     " warranty ", " fee ", " tax ", " meeting ", " verify ", _
                     " inspect ", " quote ", " discuss ", " review ")
    ' (Removed " check " and " travel ")
...
Public Function NormalizeAndAlias(ByVal text As String, ByVal aliasDict As Object) As String
    ...
    ' Replace the suffix stemming logic in NormalizeAndAlias with this:
    ...
    For sIdx = LBound(stemArr) To UBound(stemArr)
        ...
        If Len(wBody) > 4 Then
            Select Case wBody
                Case "heating", "cooling", "piping", "building", "ceiling", "wiring", "lighting"
                    ' Protect these specific industry words from being stemmed
                Case Else
...
Private Function GetCanonicalSignature(ByVal text As String, Optional ByVal isAlreadyNormalized As Boolean = False) As String
    Dim tokens() As String
    Dim i As Long, j As Long, temp As String
    
    If isAlreadyNormalized Then
...
End Function
Private Function GetVocabSignature(ByVal text As String, Optional ByVal isAlreadyNormalized As Boolean = False) As String
    Dim tokens() As String
    Dim i As Long, j As Long, temp As String
    
    ' Stop double-processing text that is already clean
    If isAlreadyNormalized Then
        tokens = Tokenize(text)
    Else
        tokens = Tokenize(NormalizeAndAlias(text, mAliasDict))
...
Private Sub EnsureDictionaries()
    ...
    mAliasDict("h p") = "horsepower"
    mAliasDict("dx") = "direct expansion"
    ...
    ' --- NEW: FIRE & LIFE SAFETY MULTI-WORD PHRASE MAPS ---
    ' Standardize complex strings into single recognizable components
    mPhraseMap("water meter") = "water flow meter"
    mPhraseMap("air conditioning systems equipment") = "terminal and package units"
    mPhraseMap("supervisory valve") = "tamper switch"
    mPhraseMap("supervisor valve") = "tamper switch"
    mPhraseMap("sprinkler valve supervisor") = "tamper switch"
    mPhraseMap("sprinkler valve") = "tamper switch"
    mPhraseMap("zone valve") = "tamper switch"
    mPhraseMap("flow valve") = "flow switch"
    mPhraseMap("dry spr") = "dry sprinkler"
    mPhraseMap("wet spr") = "wet sprinkler"

    mStripNumeric = False: mStripHash = False: mStripAlphaNum = False

    ' Load rules from external sheet
    SyncLookupsWithExternal ThisWorkbook, SHEET_LOOKUP
...
Private Function GetOrCreateListRange(ByVal wsList As Worksheet, ByVal listName As String, ByVal itemCount As Long) As Range
    ...
    ' --- BULLETPROOF ROW-LIMIT RESET ---
    If endRow > wsList.Rows.count Or startRow > wsList.Rows.count Then
...
Private Sub AppendToExternalFile(ByVal normIn As String, ByVal outP As String, ByVal sig As String)
    ...
    ' Python's /batch_learn API is now the Single Source of Truth for writing
    ' to Uniformat_Learned.txt. This prevents file-lock collisions during bulk pastes.
End Sub

' ==========================================================
' UPDATED: MigrateLocalLearningsToExternal (UTF-8 ADODB.Stream)
' ==========================================================
Public Sub MigrateLocalLearningsToExternal()
    Dim ws As Worksheet, lastRow As Long, arr As Variant
    Dim i As Long, normIn As String, outP As String, sig As String
    Dim p As String, lineStr As String, parts() As String
    ...
    ' 1. Read existing external file into memory using UTF-8 (if exists)
    If Dir(p) <> "" Then
        Set streamIn = CreateObject("ADODB.Stream")
        streamIn.Type = 2
        streamIn.Charset = "utf-8"
        streamIn.Open
        streamIn.LoadFromFile p
        
        Dim allText As String
        allText = streamIn.ReadText
        streamIn.Close
        ...
        linesArr = Split(allText, vbLf)
        
        Dim l As Long
        For l = LBound(linesArr) To UBound(linesArr)
            lineStr = linesArr(l)
    ...
    ' 3. Write back to external file using UTF-8
    ...
    For Each k In diskMap.keys
        streamOut.WriteText CStr(diskMap(k)) & vbCrLf
    Next k
...
' ==========================================================
' UPDATED: SyncRulesWithExternal (UTF-8 ADODB.Stream)
' ==========================================================
Private Function GetExternalLookupsFilePath() As String
    GetExternalLookupsFilePath = Environ("APPDATA") & "\Uniformat_Lookups.txt"
End Function

Public Sub SyncLookupsWithExternal(ByVal wb As Workbook, ByVal sheetName As String)
    Dim ws As Worksheet
    Dim p As String, lineStr As String
    Dim r As Long, lastRow As Long
    Dim streamOut As Object
    Dim c As Long
    Dim rowData(1 To 8) As String
    ...
    p = Environ("APPDATA") & "\Uniformat_Lookups.txt"
    lastRow = ws.Cells(ws.Rows.count, "A").End(xlUp).Row

    ' Use ADODB.Stream for UTF-8
    Set streamOut = CreateObject("ADODB.Stream")
    streamOut.Type = 2
    streamOut.Charset = "utf-8"
    streamOut.Open
    ...
    If lastRow >= 2 Then
        For r = 2 To lastRow
            ' Grab Columns A through H (1 to 8)
            For c = 1 To 8
                rowData(c) = Trim$(ws.Cells(r, c).text)
            Next c
            
            ' Only export if there is an actual phrase in Column A
            If Len(rowData(1)) > 0 Then
                lineStr = Join(rowData, vbTab)
                streamOut.WriteText lineStr & vbCrLf
            End If
        Next r
    End If
...
Public Sub PropagateLearnedChoice(ByVal ws As Worksheet, ByVal sourceRow As Long, ByVal targetSig As String, ByVal targetNorm As String, ByVal chosenOut As String)
    ...
    If IsError(arrData(sourceRow, 1)) Then GoTo SafeExit
    
    Dim sourceRaw As String
    sourceRaw = CStr(arrData(sourceRow, 1))
    
    ' 1. Calculate Exact Standard Key
    Dim targetClean As String
    targetClean = GetStandardKey(sourceRaw)
    
    ' 2. Calculate Core Name natively (Bypasses the slow Regex engine)
    Dim targetCore As String
    targetCore = GetCoreEquipmentName(targetClean)
    
    ' Prevent catastrophic propagation if targetCore strips down to nothing
    If Len(targetCore) < 3 Then targetCore = "|||DO_NOT_MATCH|||"

    For r = 2 To lastRow
        If r <> sourceRow Then
            If Not IsError(arrData(r, 1)) Then
                ...
                If Len(Trim$(raw)) > 0 Then
                    ...
                    rawClean = GetStandardKey(raw)
                    
                    ' Rule 1: Exact Standard Key Match
                    If rawClean = targetClean Then
                        isMatch = True
                    Else
                        ' Rule 2: Core Equipment Match (Lightning Fast - No Regex)
                        Dim rowCore As String
                        rowCore = GetCoreEquipmentName(rawClean)
                        
                        If rowCore = targetCore And Len(rowCore) >= 3 Then
                            isMatch = True
                        End If
                    End If
                    
                    If isMatch Then
                        ' Disables error handling strictly for this cell so crashes cannot spread
                        On Error Resume Next
                        
                        ws.Cells(r, 2).Validation.Delete
                        ws.Cells(r, 2).Value2 = chosenOut
                        ws.Cells(r, 3).Value2 = "Auto-Propagated"
                        ws.Range("A" & r).Interior.Color = RGB(201, 231, 167)
                        ws.Range("B" & r & ":C" & r).Interior.Color = RGB(252, 186, 186)
...
End Sub
' --- UPDATED: OMNI-DIRECTIONAL EQUIPMENT FAMILY MATCHER ---
Private Function IsSameEquipmentFamily(ByVal str1 As String, ByVal str2 As String) As Boolean
    ...
    ' If they don't have the same number of words, it is too risky to assume structural equivalence
    If UBound(arr1) <> UBound(arr2) Then Exit Function
    
    ' Cannot safely compare single-word strings using this method
    If UBound(arr1) = 0 Then Exit Function
    
    diffCount = 0
    
    ' Check how many words differ in the sequence
    For i = 0 To UBound(arr1)
        ...
        If StrComp(w1, w2, vbTextCompare) <> 0 Then
            ...
            ' If the differing words are NOT both alphanumeric tags, it's a mismatch.
            ' This prevents "Exhaust Fan 2" from matching "Supply Fan 2".
            If Not (IsDesignatorTag(w1) And IsDesignatorTag(w2)) Then
                Exit Function ' They differ on a core descriptive word
            End If
        End If
    Next i
    
    ' If differences were found, but ALL differences were just numbers/tags, they are the same family!
    If diffCount > 0 Then
        IsSameEquipmentFamily = True
    End If
End Function

' --- NEW HELPER: IDENTIFIES ASSET TAGS AND UNIT NUMBERS ---
Private Function IsDesignatorTag(ByVal txt As String) As Boolean
    Dim i As Long, ch As Integer
    
    txt = Trim$(txt)
    If Len(txt) = 0 Then Exit Function
    
    ' Condition 1: It is a very short designation (e.g., "A", "B", "CU", "f7")
    If Len(txt) <= 2 Then
        IsDesignatorTag = True
        Exit Function
    End If
    
    ' Condition 2: It contains a digit (e.g., "10th", "100A", "unit7")
    For i = 1 To Len(txt)
        ch = Asc(Mid$(txt, i, 1))
        If ch >= 48 And ch <= 57 Then ' ASCII codes for 0-9
            IsDesignatorTag = True
            Exit Function
        End If
    Next i
    
    ' If it's a long word with no numbers (e.g., "Compressor"), it is not a tag
    IsDesignatorTag = False
End Function
...
Private Function EscapeJSON(ByVal txt As String) As String
    Dim safeTxt As String
    safeTxt = txt
    ' Escape backslashes first so we don't double-escape later
    safeTxt = Replace(safeTxt, "\", "\\")
    ' Escape quotes
    safeTxt = Replace(safeTxt, """", "\""")
    ' Handle control characters
    safeTxt = Replace(safeTxt, vbCrLf, "\n")
    safeTxt = Replace(safeTxt, vbCr, "\n")
    safeTxt = Replace(safeTxt, vbLf, "\n")
...
' ==========================================================
' UPDATED: RunAISniper with safer JSON and better error handling
' ==========================================================
Public Sub RunAISniper(ws As Worksheet, rowNum As Long, inputPhrase As String)
    ...
    url = AI_API_BASE_URL & "/batch_lookup"
    ...
    If matchStr <> "" And matchStr <> "UNKNOWN" And matchStr <> "No good match" Then
        If confStr = "AI_HYBRID_MATCH" Then
            ...
            Dim fallbackArr() As String
            fallbackArr = Split(matchStr, vbLf)
            ReDim Preserve fallbackArr(LBound(fallbackArr) To UBound(fallbackArr) + 1)
            fallbackArr(UBound(fallbackArr)) = "Reject AI Guess"
            
            Call CreateSelectionListForSingleRow_Fallback(ws, rowNum, fallbackArr)
        Else
...
' ==========================================================
' THE DUMB PIPE ARCHITECTURE: RunAIBulkCleanup (ASYNCHRONOUS)
' ==========================================================
Public Sub RunAIBulkCleanup()
    Dim ws As Worksheet
    Dim lastRow As Long, i As Long
    Dim inputPhrase As String, targetCount As Long
    Dim fso As Object
    Dim tempFolder As String, inputFile As String, outputFile As String
    ...
    inputFile = tempFolder & "\ai_input.tsv"
    outputFile = tempFolder & "\ai_output.tsv"
    ...
    streamOut.WriteText "Row" & vbTab & "Phrase" & vbTab & "Manufacturer" & vbTab & "Model" & vbTab & "Serial" & vbTab & "ClientUF" & vbCrLf
    
    Dim arrIn As Variant, arrStatus As Variant
    Dim arrMfg As Variant, arrModel As Variant, arrSerial As Variant, arrUF As Variant
    
    arrIn = ws.Range("A1:A" & lastRow).Value2
    arrStatus = ws.Range("C1:C" & lastRow).Value2
    arrMfg = ws.Range("AP1:AP" & lastRow).Value2
    arrModel = ws.Range("AN1:AN" & lastRow).Value2
    arrSerial = ws.Range("AO1:AO" & lastRow).Value2
    arrUF = ws.Range("AQ1:AQ" & lastRow).Value2
    
    targetCount = 0
    For i = 2 To lastRow
        If Not IsError(arrIn(i, 1)) Then
            inputPhrase = CStr(arrIn(i, 1))
            Dim currentStatus As String
            currentStatus = CStr(arrStatus(i, 1))
            Dim needsAI As Boolean: needsAI = False
            
            If InStr(1, currentStatus, "Requires", vbTextCompare) > 0 Then needsAI = True
            If InStr(1, currentStatus, "Review", vbTextCompare) > 0 Then needsAI = True
            If InStr(1, currentStatus, "Failed", vbTextCompare) > 0 Then needsAI = True
            If InStr(1, currentStatus, "Multiple Rules", vbTextCompare) > 0 Then needsAI = True
            If InStr(1, currentStatus, "Multiple Options", vbTextCompare) > 0 Then needsAI = True
            If Len(Trim$(currentStatus)) = 0 Then needsAI = True
            
            If needsAI And Len(Trim$(inputPhrase)) > 0 Then
                targetCount = targetCount + 1
                Dim mfgStr As String, modelStr As String, serialStr As String, ufStr As String
                If Not IsError(arrMfg(i, 1)) Then mfgStr = CStr(arrMfg(i, 1)) Else mfgStr = ""
                If Not IsError(arrModel(i, 1)) Then modelStr = CStr(arrModel(i, 1)) Else modelStr = ""
                If Not IsError(arrSerial(i, 1)) Then serialStr = CStr(arrSerial(i, 1)) Else serialStr = ""
                If Not IsError(arrUF(i, 1)) Then ufStr = CStr(arrUF(i, 1)) Else ufStr = ""
                
                inputPhrase = Replace(Replace(Replace(inputPhrase, vbTab, " "), vbLf, " "), vbCr, " ")
                mfgStr = Replace(Replace(Replace(mfgStr, vbTab, " "), vbLf, " "), vbCr, " ")
                modelStr = Replace(Replace(Replace(modelStr, vbTab, " "), vbLf, " "), vbCr, " ")
                serialStr = Replace(Replace(Replace(serialStr, vbTab, " "), vbCr, " "), vbLf, " ")
                ufStr = Replace(Replace(Replace(ufStr, vbTab, " "), vbCr, " "), vbLf, " ")
                
                streamOut.WriteText i & vbTab & inputPhrase & vbTab & mfgStr & vbTab & modelStr & vbTab & serialStr & vbTab & ufStr & vbCrLf
            End If
        End If
    Next i
    
    streamOut.SaveToFile inputFile, 2
    streamOut.Close
    ...
    Dim jsonPayload As String
    jsonPayload = "{""input_file"": """ & Replace(inputFile, "\", "\\") & """, ""output_file"": """ & Replace(outputFile, "\", "\\") & """}"
    
    On Error Resume Next
    Err.Clear
    http.Open "POST", "http://127.0.0.1:5000/batch_file", False
    http.setRequestHeader "Content-Type", "application/json"
    http.send jsonPayload
    
    If Err.Number <> 0 Then
        MsgBox "VBA Connection Timeout or Error: " & vbCrLf & Err.Description, vbCritical, "Network Error"
        GoTo Cleanup
    ElseIf http.Status <> 202 And http.Status <> 200 Then
        MsgBox "Python Server Crashed!" & vbCrLf & vbCrLf & "Status Code: " & http.Status & vbCrLf & "Response: " & Left(http.responseText, 500), vbCritical, "Python Error"
        GoTo Cleanup
    End If
    On Error GoTo Cleanup
    ...
    timeoutCounter = 0
    Application.StatusBar = "AI is crunching the data... Excel is free to use while you wait."
    
    Do While Not fso.FileExists(outputFile)
        DoEvents
        Application.Wait (Now + TimeValue("0:00:01"))
        timeoutCounter = timeoutCounter + 1
        
        If timeoutCounter > 7200 Then
            MsgBox "AI processing timed out! The Python server may have crashed or encountered a fatal error.", vbCritical, "System Timeout"
            GoTo Cleanup
        End If
    Loop
    Application.StatusBar = False
    
    ' THE FIX: Give Windows time to release the file lock after Python renames it
    Application.Wait (Now + TimeValue("0:00:01"))
    
    Application.ScreenUpdating = False
    Application.EnableEvents = False
    
    Dim streamIn As Object, allOutput As String, lines() As String
    Set streamIn = CreateObject("ADODB.Stream")
    streamIn.Type = 2
    streamIn.Charset = "utf-8"
    streamIn.Open
    streamIn.LoadFromFile outputFile
    allOutput = streamIn.ReadText
    streamIn.Close
    
    lines = Split(Replace(allOutput, vbCr, ""), vbLf)
    
    Dim sheetData As Variant
    sheetData = ws.Range("B1:C" & lastRow).Value2
    
    Dim aiGuesses As Object: Set aiGuesses = CreateObject("Scripting.Dictionary")
    Dim multiRules As Object: Set multiRules = CreateObject("Scripting.Dictionary")
    Dim yellowRows As Object: Set yellowRows = CreateObject("Scripting.Dictionary")
    Dim guessRows As Object: Set guessRows = CreateObject("Scripting.Dictionary")
    
    Dim parts() As String, rRow As Long, rMatch As String, rId As String
    For i = 1 To UBound(lines)
        If Len(lines(i)) > 0 Then
            parts = Split(lines(i), vbTab)
            If UBound(parts) >= 2 Then
                rRow = CLng(val(parts(0)))
                rMatch = Replace(parts(1), "\n", vbLf)
                rId = parts(2)
                
                If rMatch <> "UNKNOWN" And rMatch <> "" And rMatch <> "SKIP" Then
                    If rId = "AI_HYBRID_MATCH" Then
                        sheetData(rRow, 1) = "Click to Review Guess..."
                        sheetData(rRow, 2) = ChrW(9888) & " Review AI Guess"
                        aiGuesses.Add CStr(rRow), rMatch
                        yellowRows(rRow) = True
                    ElseIf InStr(rMatch, vbLf) > 0 Then
                        sheetData(rRow, 1) = "Click to select rule..."
                        sheetData(rRow, 2) = rId & " (Multiple Options)"
                        multiRules.Add CStr(rRow), rMatch
                        yellowRows(rRow) = True
                    Else
                        sheetData(rRow, 1) = rMatch
                        sheetData(rRow, 2) = rId
                        If rId = "REQUIRES HUMAN" Or rMatch = "No good match" Then
                            yellowRows(rRow) = True
                        ElseIf rId = "AI_BEST_GUESS" Or rId = "EXPERT_RULE (Auto-Resolved)" Or rId = "AI_SMART_VECTOR" Then
                            guessRows(rRow) = True
                        End If
                    End If
                End If
    ...
    ws.Range("B1:C" & lastRow).Value2 = sheetData
    ws.Range("B2:B" & lastRow).Validation.Delete
    ws.Range("A2:A" & lastRow).Interior.Color = RGB(201, 231, 167)
    ws.Range("B2:C" & lastRow).Interior.Color = RGB(252, 186, 186)
    
    Dim k As Variant, mArr() As String
    
    For Each k In aiGuesses.keys
        mArr = Split(aiGuesses(k), vbLf)
        ReDim Preserve mArr(LBound(mArr) To UBound(mArr) + 1)
        mArr(UBound(mArr)) = "Reject AI Guess"
        Call CreateSelectionListForSingleRow_Fallback(ws, CLng(k), mArr)
    Next k
    ...
    For Each k In yellowRows.keys
        ws.Range("A" & k & ":C" & k).Interior.Color = RGB(255, 255, 0)
    Next k
    
    For Each k In guessRows.keys
        ws.Range("A" & k & ":C" & k).Interior.Color = RGB(255, 192, 0)
    Next k
    
    Call CalculatePMRules(ws)

Cleanup:
    On Error Resume Next
    If fso.FileExists(inputFile) Then fso.DeleteFile inputFile
    If fso.FileExists(outputFile) Then fso.DeleteFile outputFile
    Set http = Nothing ' <-- HTTP release
    On Error GoTo 0
    ...
    If Err.Number <> 0 Then
        MsgBox "An error occurred during AI processing: " & Err.Description, vbCritical
    Else
        MsgBox "Master Run Complete!", vbInformation, "AI Hand-off Complete"
    End If
End Sub
...
End Function
' ==========================================================
' UPDATED: CleanPastedOutput (Regex Loop Fix)
' ==========================================================
Public Function CleanPastedOutput(ByVal txt As String) As String
    ' Remove weird hidden characters and carriage returns, but keep line feeds (vbLf)
    ...
    For i = LBound(parts) To UBound(parts)
        ...
        If Len(parts(i)) > 0 Then
            If Len(finalStr) = 0 Then
                finalStr = parts(i)
            Else
                finalStr = finalStr & vbLf & parts(i)
            End If
        End If
    Next i
...
Public Function IsValueFromDropdown(ws As Worksheet, rowNum As Long, chosenVal As String) As Boolean
    ...
    ' 1. Reject Option Check
    If cleanChosen = "reject ai guess" Then
        IsValueFromDropdown = True
        Exit Function
    End If
    
    ' 2. Silently evaluate what the cell was before the user overwrote it
    ...
    ' 3. Multi-Rule Override Logic
    If InStr(1, confVal, "Multiple Option", vbTextCompare) > 0 Or InStr(1, confVal, "Multiple Rules", vbTextCompare) > 0 Then
        Dim parts() As String, raw As String, i As Long
        raw = Replace(outVal, vbCrLf, vbLf)
        raw = Replace(raw, vbCr, vbLf)
        parts = Split(raw, vbLf)

        For i = LBound(parts) To UBound(parts)
            ' If the pasted text exactly matches a provided multi-rule option, DO NOT learn.
        ...
        ' If the text does NOT match the options, the multi-rule was wrong.
        ' The user pasted a manual override. We MUST learn it.
        IsValueFromDropdown = False
        Exit Function
    End If

    ' 4. Always learn AI Guesses or completely unmapped overrides
    IsValueFromDropdown = False
End Function
' ==========================================================
' NEW: Batched API Payload (Replaces TeachPythonAI)
' ==========================================================
Public Sub FlushBatchLearningsToPython()
    If mBatchLearnDict Is Nothing Then Exit Sub
    If mBatchLearnDict.count = 0 Then Exit Sub
    ...
    json = "{""items"": ["
    i = 0
    For Each k In mBatchLearnDict.keys
        If i > 0 Then json = json & ","
        json = json & "{""phrase"": """ & EscapeJSON(CStr(k)) & """, ""match"": """ & EscapeJSON(CStr(mBatchLearnDict(k))) & """}"
        i = i + 1
    Next k
    json = json & "]}"
    ...
    http.Open "POST", AI_API_BASE_URL & "/batch_learn", False
    http.setRequestHeader "Content-Type", "application/json"
    http.send json
    On Error GoTo 0
    
    Set mBatchLearnDict = CreateObject("Scripting.Dictionary")
    Set http = Nothing
End Sub
...
Public Function BuildSearchDropdown(wsInput As Worksheet, rowNum As Long, searchStr As String) As Boolean
    ...
    Dim i As Long
    Dim sClean As String: sClean = GetStandardKey(searchStr)
...
Private Sub FlushMemory()
    ' Clear all module-level dictionaries to prevent RAM bloat
    Set mLookupPhraseSet = Nothing
    Set mAliasDict = Nothing
    Set mStopWords = Nothing
    Set mBoostDict = Nothing
    Set mCoreSet = Nothing
    Set mQualSet = Nothing
    Set mPhraseMap = Nothing
    Set mForcedOutputDict = Nothing
    Set mProtectSet = Nothing
    Set mAliasPrefix = Nothing
    Set mAliasSuffix = Nothing
    Set mTokenIndex = Nothing
    Set mLookupVocab = Nothing
    Set mTokenWeights = Nothing
    Set mTokenCatFreq = Nothing
    ...
    Set mLearnedSigCount = Nothing
    Set mLearnedVocabSigBest = Nothing
    
    ' Erase module-level arrays
    Erase mLookupPhrases
    Erase mLookupWords
    Erase mLookupDicts
    Erase mLookupSubjects
    Erase mRowCategory
    Erase mLookupWordCounts
    Erase mCategoryKeys
    Erase mLearnedSubstKeys
    
    mLookupCount = 0
    mCategoryCount = 0
    mLearnedLoaded = False
End Sub

Public Sub UnstickExcel()
    Application.EnableEvents = True
    Application.ScreenUpdating = True
    Application.Calculation = xlCalculationAutomatic
    MsgBox "Excel is listening again!", vbInformation
End Sub

Public Sub ReloadPythonAI()
    Dim http As Object
    Dim url As String
    ...
    If http.Status = 200 Then
        MsgBox "AI Knowledge Base Successfully Reloaded!", vbInformation, "System Update"
    Else
        MsgBox "Failed to reload AI. Is the Python server running?", vbCritical, "System Error"
    End If
    
    Set http = Nothing
    On Error GoTo 0
End Sub
Public Sub RebootVBAMemory()
    Dim ws As Worksheet
    
    ' Target the Uniformat Lookup sheet specifically
    On Error Resume Next
    Set ws = ThisWorkbook.Worksheets("Uniformat RS Means Lookup")
    On Error GoTo 0
    
    If ws Is Nothing Then
        MsgBox "Cannot find the Uniformat Lookup sheet! Check the tab name.", vbCritical
        Exit Sub
    End If
    
    ' Force the dictionary to dump and rebuild from scratch
    Application.Cursor = xlWait
    Call InitializeMatcher(ws)
    Application.Cursor = xlDefault
    
    MsgBox "VBA Memory and Dictionaries Successfully Rebooted!", vbInformation, "System Restored"
End Sub

' ==========================================================
' ASYNCHRONOUS DATA PRE-SCRUB
' ==========================================================
Public Sub RunDataPreScrub()
    Dim wsSource As Worksheet
    Dim wsScrub As Worksheet
    Dim lastRow As Long, i As Long
    Dim inputPhrase As String, buildingStr As String
    Dim mfgStr As String, modelStr As String, serialStr As String, ufStr As String
    Dim fso As Object
    Dim tempFolder As String, inputFile As String, outputFile As String
    ...
    Set wsSource = ThisWorkbook.Worksheets("Helper Sheet")
    lastRow = wsSource.Cells(wsSource.Rows.count, "A").End(xlUp).Row
    If lastRow < 2 Then Exit Sub
    
    Set fso = CreateObject("Scripting.FileSystemObject")
    tempFolder = Environ("TEMP")
    inputFile = tempFolder & "\prescrub_in.tsv"
    outputFile = tempFolder & "\prescrub_out.tsv"
    ...
    streamOut.WriteText "Row" & vbTab & "Building" & vbTab & "Phrase" & vbTab & "Manufacturer" & vbTab & "Model" & vbTab & "Serial" & vbTab & "ClientUF" & vbCrLf
    
    Dim arrIn As Variant, arrBldg As Variant, arrMfg As Variant, arrModel As Variant, arrSerial As Variant, arrUF As Variant
    arrIn = wsSource.Range("A1:A" & lastRow).Value2
    arrBldg = wsSource.Range("D1:D" & lastRow).Value2
    arrMfg = wsSource.Range("AP1:AP" & lastRow).Value2
    arrModel = wsSource.Range("AN1:AN" & lastRow).Value2
    arrSerial = wsSource.Range("AO1:AO" & lastRow).Value2
    arrUF = wsSource.Range("AQ1:AQ" & lastRow).Value2
    
    For i = 2 To lastRow
        inputPhrase = "": buildingStr = "": mfgStr = "": modelStr = "": serialStr = "": ufStr = ""
        If Not IsError(arrIn(i, 1)) Then inputPhrase = CStr(arrIn(i, 1))
        If Not IsError(arrBldg(i, 1)) Then buildingStr = CStr(arrBldg(i, 1))
        If Not IsError(arrMfg(i, 1)) Then mfgStr = CStr(arrMfg(i, 1))
        If Not IsError(arrModel(i, 1)) Then modelStr = CStr(arrModel(i, 1))
        If Not IsError(arrSerial(i, 1)) Then serialStr = CStr(arrSerial(i, 1))
        If Not IsError(arrUF(i, 1)) Then ufStr = CStr(arrUF(i, 1))
        
        inputPhrase = Replace(Replace(Replace(inputPhrase, vbTab, " "), vbLf, " "), vbCr, " ")
        buildingStr = Replace(Replace(Replace(buildingStr, vbTab, " "), vbLf, " "), vbCr, " ")
        mfgStr = Replace(Replace(Replace(mfgStr, vbTab, " "), vbLf, " "), vbCr, " ")
        modelStr = Replace(Replace(Replace(modelStr, vbTab, " "), vbLf, " "), vbCr, " ")
        serialStr = Replace(Replace(Replace(serialStr, vbTab, " "), vbCr, " "), vbLf, " ")
        ufStr = Replace(Replace(Replace(ufStr, vbTab, " "), vbLf, " "), vbCr, " ")
        
        streamOut.WriteText i & vbTab & buildingStr & vbTab & inputPhrase & vbTab & mfgStr & vbTab & modelStr & vbTab & serialStr & vbTab & ufStr & vbCrLf
    Next i
    ...
    On Error Resume Next
    Err.Clear
    http.Open "POST", "http://127.0.0.1:5000/prescrub_file", False
    http.setRequestHeader "Content-Type", "application/json"
    http.send jsonPayload
    ...
    Dim timeoutCounter As Long
    timeoutCounter = 0
    Application.StatusBar = "AI is Pre-Scrubbing the data... Excel is free to use while you wait."
    ...
    On Error GoTo 0
    If wsScrub Is Nothing Then
        Set wsScrub = ThisWorkbook.Worksheets.Add(After:=wsSource)
        wsScrub.Name = "Scrubbed Inputs"
    End If
    
    wsScrub.Cells.Clear
    wsScrub.Range("A1:Q1").Value = Array("Row", "Building", "Original Phrase", "Scrubbed Phrase", "LLM Model Identity", "Technical Specs", "Consumables", "Lifecycle Data", "Asset Tag", "Location", "System Group", "Hierarchy", "Audit Flag", "PM Suitability", "Yearly Effort (Hrs)", "Yearly Material ($)", "NFPA Group")
    wsScrub.Range("A1:Q1").Font.Bold = True
    wsScrub.Range("A1:Q1").Interior.Color = RGB(220, 230, 241)
    ...
    ' --- THE ALIGNMENT FIX ---
    ' Size the array perfectly to the Helper Sheet dimensions
    Dim writeData() As Variant
    ReDim writeData(1 To lastRow - 1, 1 To 17)
    ...
    For i = 1 To UBound(lines)
        If Len(lines(i)) > 0 Then
            parts = Split(lines(i), vbTab)
            If UBound(parts) >= 16 Then
                If IsNumeric(parts(0)) Then
                    ' Retrieve the exact Helper Sheet row number from the scrambled output
                    targetRow = CLng(val(parts(0)))
                    
                    If targetRow >= 2 And targetRow <= lastRow Then
                        ' Map Excel Row 2 to Array Index 1, Row 3 to Index 2, etc.
                        arrIdx = targetRow - 1
                        
                        Dim c As Long
                        For c = 0 To 16
                            ' Assign the data to its absolute index to enforce 1-to-1 alignment
                            ' --- NEW FIX: Unescape the line breaks to restore multiple options ---
                            writeData(arrIdx, c + 1) = Replace(parts(c), "\n", vbLf)
                        Next c
                    End If
                End If
            End If
    ...
    ' Print the perfectly aligned array block back into Excel
    wsScrub.Range("A2").Resize(lastRow - 1, 17).Value2 = writeData
    wsScrub.Columns("A:Q").AutoFit
    
    Dim lastScrub As Long: lastScrub = lastRow
    
    wsScrub.Range("L2:L" & lastScrub).FormatConditions.Add Type:=xlTextString, String:="??", TextOperator:=xlContains
    wsScrub.Range("L2:L" & lastScrub).FormatConditions(1).Interior.Color = RGB(255, 235, 156)
    wsScrub.Range("L2:L" & lastScrub).FormatConditions(1).Font.Color = RGB(156, 101, 0)
    
    wsScrub.Range("L2:L" & lastScrub).FormatConditions.Add Type:=xlTextString, String:="Matched", TextOperator:=xlContains
    wsScrub.Range("L2:L" & lastScrub).FormatConditions(2).Interior.Color = RGB(198, 239, 206)
    wsScrub.Range("L2:L" & lastScrub).FormatConditions(2).Font.Color = RGB(0, 97, 0)
    
    wsScrub.Range("L2:L" & lastScrub).FormatConditions.Add Type:=xlTextString, String:="Valid", TextOperator:=xlContains
    wsScrub.Range("L2:L" & lastScrub).FormatConditions(3).Interior.Color = RGB(198, 239, 206)
    wsScrub.Range("L2:L" & lastScrub).FormatConditions(3).Font.Color = RGB(0, 97, 0)
    
    Dim scrubArr As Variant: scrubArr = wsScrub.Range("A2:N" & lastScrub).Value2
    Dim pmArr As Variant: ReDim pmArr(1 To lastRow - 1, 1 To 1)
    Dim rIdx As Long, helperRow As Long, rawFlag As String
    
    For rIdx = 1 To UBound(scrubArr, 1)
        If IsNumeric(scrubArr(rIdx, 1)) Then
            helperRow = CLng(val(scrubArr(rIdx, 1)))
            If helperRow >= 2 And helperRow <= lastRow Then
                rawFlag = CStr(scrubArr(rIdx, 13))
                If InStr(1, rawFlag, "??", vbTextCompare) > 0 Or InStr(1, rawFlag, "Missing", vbTextCompare) > 0 Then
                    pmArr(helperRow - 1, 1) = "Valid System (See Scrubbed Inputs)"
                Else
                    pmArr(helperRow - 1, 1) = rawFlag
                End If
            End If
        End If
    Next rIdx
    
    wsSource.Range("F2:F" & lastRow).Value2 = pmArr
...
Cleanup:
    ...
    Call BuildTriageBoard
    Call BuildMissingParentSummary
    Call CalculatePMRules(wsSource)
    ...
    If Err.Number <> 0 Then
        MsgBox "An error occurred during Pre-Scrub processing: " & Err.Description, vbCritical
    Else
        wsScrub.Activate
        MsgBox "Pre-Scrub & LLM Identity Complete! Review your data.", vbInformation, "Data Staging"
    End If
End Sub
' ==========================================================
' PARENT / CHILD PM TRIGGER LOGIC WITH LIVE DASHBOARD
' ==========================================================
Public Sub CalculatePMRules(ByVal ws As Worksheet, Optional ByVal showPopup As Boolean = False)
    Dim lastRow As Long
    lastRow = ws.Cells(ws.Rows.count, "A").End(xlUp).Row
    If lastRow < 2 Then Exit Sub
    
    Dim overrideDict As Object
    Set overrideDict = CreateObject("Scripting.Dictionary")
    ...
    ' Dictionary to store Scrubbed Inputs data mapped by Row ID
    Dim scrubDataDict As Object
    Set scrubDataDict = CreateObject("Scripting.Dictionary")
    
    If Not wsScrub Is Nothing Then
        Dim tLast As Long
        tLast = wsScrub.Cells(wsScrub.Rows.count, "A").End(xlUp).Row
        If tLast >= 2 Then
            Dim tArr As Variant
            tArr = wsScrub.Range("A2:Q" & tLast).Value2
            Dim tIdx As Long
            For tIdx = 1 To UBound(tArr, 1)
                Dim rId As String
                rId = CStr(tArr(tIdx, 1)) ' Column A: Exact Excel Row Number
                
                ' ONLY map if the Row ID is a valid number matching the Helper Sheet row
                If IsNumeric(rId) Then
                    Dim excelRowKey As Long
                    excelRowKey = CLng(rId)
                    ' Map data directly to its exact numeric row key
                    scrubDataDict(excelRowKey) = Array(CStr(tArr(tIdx, 11)), CStr(tArr(tIdx, 17)), CStr(tArr(tIdx, 13)))
                End If
            Next tIdx
        End If
        
        ' Also grab Triage overrides
        tLast = wsScrub.Cells(wsScrub.Rows.count, "X").End(xlUp).Row
        If tLast >= 2 Then
            For tIdx = 2 To tLast
                Dim tBldg As String, tAsset As String, tChoice As String
                tBldg = UCase(Trim(CStr(wsScrub.Cells(tIdx, "X").Value2)))
                tAsset = LCase(Trim(CStr(wsScrub.Cells(tIdx, "Y").Value2)))
                tChoice = Trim(CStr(wsScrub.Cells(tIdx, "Z").Value2))
                If tChoice <> "" And tChoice <> "Select System..." And tChoice <> "-" Then
                    overrideDict(tBldg & "|" & tAsset) = tChoice
                End If
            Next tIdx
        End If
    End If
    
    Dim arrBldg As Variant, arrAsset As Variant, arrPM As Variant, arrFeasibility As Variant
    arrAsset = ws.Range("B1:B" & lastRow).Value2
    arrBldg = ws.Range("D1:D" & lastRow).Value2
    
    ReDim arrPM(1 To lastRow, 1 To 1)
    ReDim arrFeasibility(1 To lastRow, 1 To 1)
    arrPM(1, 1) = "PM Trigger"
    arrFeasibility(1, 1) = "PM Feasibility"
    
    Dim parentsDict As Object, pCountDict As Object, cCountDict As Object
    Set parentsDict = CreateObject("Scripting.Dictionary")
    Set pCountDict = CreateObject("Scripting.Dictionary")
    Set cCountDict = CreateObject("Scripting.Dictionary")
    
    Dim missingParentsDict As Object
    Set missingParentsDict = CreateObject("Scripting.Dictionary")
    
    Dim i As Long, grp As String, role As String, bldg As String, asset As String
    Dim dictKey As String, overrideKey As String, pmLabel As String, nfpaGroup As String
    
    For i = 2 To lastRow
        If Not IsError(arrAsset(i, 1)) And Not IsError(arrBldg(i, 1)) Then
            asset = CStr(arrAsset(i, 1))
            bldg = UCase(Trim(CStr(arrBldg(i, 1))))
            
            If Len(asset) > 0 And Len(bldg) > 0 Then
                Call GetHierarchyInfo(asset, grp, role)
                
                overrideKey = bldg & "|" & LCase(Trim(asset))
                If overrideDict.Exists(overrideKey) Then
                    grp = overrideDict(overrideKey)
                    role = "Child"
                End If
                
                If grp <> "" And grp <> "Ambiguous Fuel System" Then
                    dictKey = bldg & "|" & grp
                    If role = "Parent" Then
                        parentsDict(dictKey) = True
                        pCountDict(dictKey) = pCountDict(dictKey) + 1
                    ElseIf role = "Child" Then
                        cCountDict(dictKey) = cCountDict(dictKey) + 1
                    End If
                End If
            End If
        End If
    ...
    For i = 2 To lastRow
        If Not IsError(arrAsset(i, 1)) Then
            asset = CStr(arrAsset(i, 1))
            If Not IsError(arrBldg(i, 1)) Then bldg = UCase(Trim(CStr(arrBldg(i, 1)))) Else bldg = ""
            ...
            If Len(asset) > 0 Then
                ...
                ' Look up matching data from scrubbed inputs using the Excel Row index (i)
                nfpaGroup = ""
                If scrubDataDict.Exists(i) Then
                    Dim dataPair As Variant
                    dataPair = scrubDataDict(i)
                    nfpaGroup = CStr(dataPair(1))
                End If
                
                If role = "Child" And grp <> "Ambiguous Fuel System" Then
                    If Len(bldg) > 0 Then
                        If parentsDict.Exists(bldg & "|" & grp) Then
                            pmLabel = ""
                            Select Case grp
                                Case "Life Safety / Fire"
                                    pmLabel = " - Group 1: Fire Alarm & Detection (NFPA 72)"
                                Case "Fire Pump System", "Fire Sprinkler"
                                    pmLabel = " - Group 2: Water-Based & Active Suppression (NFPA 25 / 17A)"
                                Case Else
                                    pmLabel = " - " & grp
                            End Select
                            
                            arrPM(i, 1) = "0" & pmLabel
                            arrFeasibility(i, 1) = "Valid System (Parent Exists)"
                        Else
                            arrPM(i, 1) = 1
                            arrFeasibility(i, 1) = "?? " & grp & ": Missing Parent Asset" & IIf(Len(nfpaGroup) > 0, " - " & nfpaGroup, "")
                            
                            Dim auditKey As String: auditKey = bldg & "|" & grp
                            If Not missingParentsDict.Exists(auditKey) Then
                                missingParentsDict(auditKey) = 1
                            Else
                                missingParentsDict(auditKey) = CLng(missingParentsDict(auditKey)) + 1
                            End If
                        End If
                    Else
                        arrPM(i, 1) = 1
                        arrFeasibility(i, 1) = "Valid System"
                    End If
                Else
                    arrPM(i, 1) = 1
                    If Len(nfpaGroup) > 0 Then
                        If InStr(1, nfpaGroup, "Group 3", vbTextCompare) > 0 Then
                            arrFeasibility(i, 1) = "Valid Passive Fire Asset - " & nfpaGroup
                        Else
                            arrFeasibility(i, 1) = "Valid System - " & nfpaGroup
                        End If
                    Else
                        ' --- NEW FIX: Native VBA Fallback for Auto-Propagated Parents ---
                        Dim parentNfpa As String
                        parentNfpa = ""
                        
                        Select Case grp
                            Case "Life Safety / Fire"
                                parentNfpa = "Group 1: Fire Alarm & Detection (NFPA 72)"
                            Case "Fire Pump System", "Fire Sprinkler"
                                parentNfpa = "Group 2: Water-Based & Active Suppression (NFPA 25 / 17A)"
                            Case "Passive Fire Defenses"
                                parentNfpa = "Group 3: Passive Fire Defenses & Portable Equipment (NFPA 80 / 10)"
                        End Select
                        
                        If Len(parentNfpa) > 0 Then
                            If InStr(1, parentNfpa, "Group 3", vbTextCompare) > 0 Then
                                arrFeasibility(i, 1) = "Valid Passive Fire Asset - " & parentNfpa
                            Else
                                arrFeasibility(i, 1) = "Valid System - " & parentNfpa
                            End If
                        Else
                            arrFeasibility(i, 1) = "Valid System"
                        End If
    ...
    Application.EnableEvents = False
    ws.Range("E1:E" & lastRow).Value2 = arrPM
    ws.Range("F1:F" & lastRow).Value2 = arrFeasibility
    Application.EnableEvents = True
    
    If showPopup = True Then
        If missingParentsDict.count > 0 Then
            Dim auditMsg As String, k As Variant, parts() As String
            auditMsg = "?? AUDIT ALERT: Missing Parent Assets Detected!" & vbCrLf & vbCrLf
            Dim countLimit As Long: countLimit = 0
            For Each k In missingParentsDict.keys
                parts = Split(CStr(k), "|")
                auditMsg = auditMsg & "• Building [" & parts(0) & "] missing Parent for [" & parts(1) & "] (" & missingParentsDict(k) & " child devices)" & vbCrLf
                countLimit = countLimit + 1
                If countLimit >= 12 Then
                    auditMsg = auditMsg & "... (and " & (missingParentsDict.count - 12) & " more system issues)" & vbCrLf
                    Exit For
                End If
            Next k
            MsgBox auditMsg, vbExclamation, "PM Hierarchy Audit Check"
        Else
            MsgBox "All children have valid parent assets!", vbInformation, "PM Hierarchy Audit Check"
        End If
    End If
End Sub


Private Sub GetHierarchyInfo(ByVal assetType As String, ByRef outGroup As String, ByRef outRole As String)
    outGroup = "": outRole = ""
    Dim s As String: s = " " & LCase(Trim(assetType)) & " "
    
    ' --- VERTICAL TRANSPORT ---
    If InStr(s, " elevator controller") > 0 Or InStr(s, " hoistway") > 0 Or InStr(s, " pit equipment") > 0 Or InStr(s, " machine room") > 0 Then
        outGroup = "Vertical Transport": outRole = "Child": Exit Sub
    End If
    If InStr(s, " elevator") > 0 Or InStr(s, " cab") > 0 Or InStr(s, " escalator") > 0 Or InStr(s, " moving walk") > 0 Or InStr(s, " wheelchair lift") > 0 Or InStr(s, " dumbwaiter") > 0 Then
        outGroup = "Vertical Transport": outRole = "Parent": Exit Sub
    End If
    
    ' --- HYDRONIC HVAC ---
    If InStr(s, " hydronic fan coil") > 0 Then
        outGroup = "Hydronic HVAC": outRole = "Parent": Exit Sub
    End If
    
    ' --- SPLIT HVAC ---
    If InStr(s, " fan coil") > 0 Or InStr(s, " fcu") > 0 Or InStr(s, " evaporator") > 0 Or InStr(s, " vrf indoor") > 0 Or InStr(s, " vrv indoor") > 0 Or InStr(s, " cassette") > 0 Or InStr(s, " wall mount split") > 0 Or InStr(s, " indoor unit") > 0 Then
        outGroup = "Split HVAC": outRole = "Child": Exit Sub
    End If
    If InStr(s, " condensing unit") > 0 Or InStr(s, " condenser") > 0 Or InStr(s, " cu") > 0 Or InStr(s, " outdoor unit") > 0 Or InStr(s, " vrf outdoor") > 0 Or InStr(s, " vrv outdoor") > 0 Or InStr(s, " heat pump outdoor") > 0 Then
        outGroup = "Split HVAC": outRole = "Parent": Exit Sub
    End If
    
    ' --- FIRE PUMP ---
    If InStr(s, " jockey pump") > 0 Or InStr(s, " pressure maintenance pump") > 0 Or InStr(s, " fire pump controller") > 0 Then
        outGroup = "Fire Pump System": outRole = "Child": Exit Sub
    End If
    If InStr(s, " fire pump") > 0 Or InStr(s, " electric fire pump") > 0 Or InStr(s, " diesel fire pump") > 0 Then
        outGroup = "Fire Pump System": outRole = "Parent": Exit Sub
    End If
    
    ' --- LIFE SAFETY / FIRE ALARM ---
    ' Removed fire damper and fire box so they don't get zeroed out
    If InStr(s, " smoke") > 0 Or InStr(s, " detector") > 0 Or InStr(s, " strobe") > 0 Or InStr(s, " horn") > 0 Or InStr(s, " pull") > 0 Or InStr(s, " bell") > 0 Or InStr(s, " tamper switch") > 0 Or InStr(s, " flow switch") > 0 Or InStr(s, " interface device") > 0 Then
        outGroup = "Life Safety / Fire": outRole = "Child": Exit Sub
    End If
    If InStr(s, " fire alarm control panel") > 0 Or InStr(s, " fire panel") > 0 Or InStr(s, " facp") > 0 Or InStr(s, " annunciator") > 0 Or InStr(s, " main fire") > 0 Then
        outGroup = "Life Safety / Fire": outRole = "Parent": Exit Sub
    End If
    
    ' --- PASSIVE FIRE & ROUTE ASSETS (NFPA 80 / 10) ---
    ' Forcing these to standalone so they receive a PM Trigger of 1
    If InStr(s, " fire damper") > 0 Or InStr(s, " fire door") > 0 Or InStr(s, " fire extinguisher") > 0 Or InStr(s, " fire box") > 0 Or InStr(s, " hose cabinet") > 0 Then
        outGroup = "Passive Fire Defenses": outRole = "Standalone": Exit Sub
    End If
    
    ' --- FIRE SPRINKLER ---
    If InStr(s, " sprinkler head") > 0 Or InStr(s, " drop") > 0 Or InStr(s, " pressure reducing valve") > 0 Or InStr(s, " prv") > 0 Or InStr(s, " hose valve") > 0 Then
        outGroup = "Fire Sprinkler": outRole = "Child": Exit Sub
    End If
    If InStr(s, " sprinkler system") > 0 Or InStr(s, " wet pipe") > 0 Or InStr(s, " dry pipe") > 0 Or InStr(s, " pre-action") > 0 Or InStr(s, " preaction") > 0 Or InStr(s, " deluge") > 0 Or InStr(s, " fire water storage tank") > 0 Or InStr(s, " fire tank") > 0 Or InStr(s, " standpipe system") > 0 Or InStr(s, " standpipe") > 0 Then
        outGroup = "Fire Sprinkler": outRole = "Parent": Exit Sub
    End If
    
    ' --- EMERGENCY POWER ---
    If InStr(s, " ats") > 0 Or InStr(s, " automatic transfer switch") > 0 Or InStr(s, " transfer switch") > 0 Or InStr(s, " load bank") > 0 Then
        outGroup = "Emergency Power": outRole = "Child": Exit Sub
    End If
    If InStr(s, " generator") > 0 Or InStr(s, " genset") > 0 Or InStr(s, " diesel gen") > 0 Or InStr(s, " gas gen") > 0 Then
        outGroup = "Emergency Power": outRole = "Parent": Exit Sub
    End If
    
    ' --- AMBIGUOUS FUEL SYSTEM (Quarantine) ---
    If InStr(s, " day tank") > 0 Or InStr(s, " fuel polishing") > 0 Or InStr(s, " fuel tank") > 0 Or InStr(s, " fuel oil tank") > 0 Or InStr(s, " transfer pump") > 0 Then
        outGroup = "Ambiguous Fuel System": outRole = "Child": Exit Sub
    End If
    
    ' --- WATER COOLED HEAT PUMP ---
    If InStr(s, " water cooled heat pump") > 0 Then
        outGroup = "Water Cooled Heat Pump": outRole = "Parent": Exit Sub
    End If
End Sub

Public Sub BuildTriageBoard()
    Dim wsScrub As Worksheet
    On Error Resume Next
    Set wsScrub = ThisWorkbook.Worksheets("Scrubbed Inputs")
    On Error GoTo 0
    If wsScrub Is Nothing Then Exit Sub
    
    ' SHIFTED BOARD TO X:Z
    wsScrub.Range("X:Z").Clear
    wsScrub.Range("X:Z").Validation.Delete
    
    wsScrub.Cells(1, "X").Value2 = "BUILDING"
    wsScrub.Cells(1, "Y").Value2 = "AMBIGUOUS FUEL ASSET"
    wsScrub.Cells(1, "Z").Value2 = "ASSIGN TO SYSTEM"
    With wsScrub.Range("X1:Z1")
        .Font.Bold = True
    ...
    Dim lastRow As Long
    lastRow = wsScrub.Cells(wsScrub.Rows.count, "A").End(xlUp).Row
    If lastRow < 2 Then Exit Sub
    
    Dim arrData As Variant
    arrData = wsScrub.Range("A1:K" & lastRow).Value2
    
    Dim i As Long, printRow As Long
    printRow = 2
    
    For i = 2 To lastRow
        If Trim(CStr(arrData(i, 11))) = "Ambiguous Fuel System" Then
            wsScrub.Cells(printRow, "X").Value2 = Trim(CStr(arrData(i, 2))) ' Building
            wsScrub.Cells(printRow, "Y").Value2 = Trim(CStr(arrData(i, 3))) ' Phrase
            wsScrub.Cells(printRow, "Z").Value2 = "Select System..."
            
            With wsScrub.Cells(printRow, "Z").Validation
                .Delete
                .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Operator:= _
                xlBetween, Formula1:="Emergency Power,Fire Pump System,Boiler System"
                .IgnoreBlank = True
                .InCellDropdown = True
            End With
            
            wsScrub.Range("X" & printRow & ":Z" & printRow).Interior.Color = RGB(255, 242, 204)
            printRow = printRow + 1
        End If
    Next i
    
    If printRow = 2 Then
        wsScrub.Cells(2, "X").Value2 = "-"
        wsScrub.Cells(2, "Y").Value2 = "No ambiguous assets found."
        wsScrub.Cells(2, "Z").Value2 = "-"
        wsScrub.Range("X2:Z2").Interior.Color = RGB(201, 231, 167)
    End If
    
    wsScrub.Columns("X:Z").AutoFit
End Sub

Public Sub BuildMissingParentSummary()
    Dim ws As Worksheet
    On Error Resume Next
    Set ws = ThisWorkbook.Worksheets("Scrubbed Inputs")
    On Error GoTo 0
    ...
    ' SHIFTED BOARD TO S:V
    ws.Range("S:V").Clear
    
    If lastRow < 2 Then Exit Sub
    
    Dim arrData As Variant
    arrData = ws.Range("A1:M" & lastRow).Value2
    
    Dim i As Long
    Dim bldg As String, flag As String
    Dim missingDict As Object
    Set missingDict = CreateObject("Scripting.Dictionary")
    
    For i = 2 To lastRow
        bldg = Trim(CStr(arrData(i, 2)))
        flag = Trim(CStr(arrData(i, 13)))
        
        If InStr(1, flag, "Missing", vbTextCompare) > 0 Then
            Dim dictKey As String
            dictKey = bldg & "|" & flag
            If Not missingDict.Exists(dictKey) Then
                missingDict(dictKey) = 1
            Else
                missingDict(dictKey) = missingDict(dictKey) + 1
            End If
        End If
    Next i
    
    ws.Cells(1, "S").Value2 = "BUILDING"
    ws.Cells(1, "T").Value2 = "MISSING PARENT ALERT"
    ws.Cells(1, "U").Value2 = "CHILDREN IMPACTED"
    ws.Cells(1, "V").Value2 = "Spacer"
    ...
    If missingDict.count = 0 Then
        ws.Cells(printRow, "S").Value2 = "All Buildings"
        ws.Cells(printRow, "T").Value2 = "All Children Have Parents!"
        ws.Cells(printRow, "U").Value2 = "-"
        ws.Range("S2:U2").Interior.Color = RGB(201, 231, 167)
    Else
        Dim k As Variant
        Dim parts() As String
        For Each k In missingDict.keys
            parts = Split(CStr(k), "|")
            ws.Cells(printRow, "S").Value2 = parts(0)
            ws.Cells(printRow, "T").Value2 = parts(1)
            ws.Cells(printRow, "U").Value2 = missingDict(k)
            
            ws.Range("S" & printRow & ":U" & printRow).Interior.Color = RGB(252, 186, 186)
            printRow = printRow + 1
        Next k
    End If
    
    ws.Columns("S:V").AutoFit
End Sub
Public Sub RunManualPMAudit()
    ' Runs the calculation and forces the popup to show
    Call CalculatePMRules(ActiveSheet, True)
End Sub

Public Sub RealignDropdownsAfterSort()
    Dim ws As Worksheet
    Dim lastRow As Long, i As Long
    ...
    ' 1. Wipe the orphaned background lists
    Call CleanupOrphanedPickLists
    
    ' 2. Rebuild the Multi-Select Dropdowns directly from the cell text
    For i = 2 To lastRow
        raw = CStr(ws.Cells(i, "B").Value2)
        If InStr(1, raw, vbLf) > 0 Then
            parts = Split(raw, vbLf)
            If UBound(parts) > 0 Then
                Call CreateSelectionListForSingleRow_Fallback(ws, i, parts)
            End If
        End If
    Next i
    ...
    MsgBox "Multi-select dropdowns successfully realigned to match your new sorted order!", vbInformation, "Sort Fix Complete"
End Sub

' ==========================================================
' OVERWRITE LEARNED RULES (Handles Deletions)
' ==========================================================
Public Sub OverwriteLearnedFromSheet()
    Dim ws As Worksheet, lastRow As Long
    Dim streamOut As Object
    Dim p As String, i As Long
    Dim normIn As String, outP As String, sig As String, tStamp As String
    ...
    ' Open a clean UTF-8 text stream
    Set streamOut = CreateObject("ADODB.Stream")
    streamOut.Type = 2
    streamOut.Charset = "utf-8"
    streamOut.Open

    ' Write only what currently exists on the sheet
    If lastRow >= 2 Then
        For i = 2 To lastRow
            normIn = CStr(ws.Cells(i, 1).Value2)
            outP = CStr(ws.Cells(i, 2).Value2)
            sig = CStr(ws.Cells(i, 3).Value2)
            tStamp = CStr(ws.Cells(i, 5).Value2)
            
            If Len(normIn) > 0 And Len(outP) > 0 Then
                If Len(tStamp) = 0 Then tStamp = Now
                streamOut.WriteText normIn & "|" & outP & "|" & sig & "|" & tStamp & vbCrLf
            End If
        Next i
    End If

    ' Force SaveCreateOverWrite (2)
    streamOut.SaveToFile p, 2
    streamOut.Close
End Sub

' ==========================================================
' THE "SYNC NOW" MASTER BUTTON
' ==========================================================
Public Sub ForceMasterSync()
    Application.ScreenUpdating = False
    Application.Cursor = xlWait
    
    ' --- NEW LINE: Force hard drive to match active memory before syncing ---
    ' ThisWorkbook.Save
    
    ' 1. Force overwrite of all AppData files from the current Excel sheets
    Call SyncLookupsWithExternal(ThisWorkbook, SHEET_LOOKUP)
    Call SyncRulesWithExternal(ThisWorkbook, SHEET_RULES)
    Call OverwriteLearnedFromSheet
    
    ' 2. Re-initialize the VBA memory so local dictionaries match
    Dim wsLookup As Worksheet
    On Error Resume Next
    Set wsLookup = ThisWorkbook.Worksheets(SHEET_LOOKUP)
    On Error GoTo 0
    If Not wsLookup Is Nothing Then Call InitializeMatcher(wsLookup)
    
    ' 3. Ping Python to reload the freshly written files
    Call ReloadPythonAI
    
    Application.Cursor = xlDefault
    Application.ScreenUpdating = True
    
    MsgBox "Manual Sync Complete!" & vbCrLf & vbCrLf & _
           "All rules, lookups, and deletions have been successfully pushed to the AI.", _
           vbInformation, "Sync Master"
End Sub

' ==========================================================
' NEW: Aggressively strips facility coordinates to find the base asset
' ==========================================================
Private Function GetCoreEquipmentName(ByVal text As String) As String
    Dim parts() As String, i As Long
    Dim out As String, w As String
    parts = Split(text, " ")
    
    For i = LBound(parts) To UBound(parts)
        w = LCase(Trim(parts(i)))
        
        If Len(w) <= 1 Then GoTo SkipWord
        If IsNumeric(w) Or HasAnyDigit(w) Or IsTagLike(w) Then GoTo SkipWord
        
        ' --- FIX: Expanded noise-word stripping for location/architectural descriptors ---
        Select Case w
            Case "north", "south", "east", "west", "ne", "nw", "se", "sw", "n", "s", "e", "w", _
                 "up", "down", "upper", "lower", "left", "right", _
                 "central", "middle", "front", "back", "center", "interior", "exterior", "peripheral", _
                 "int", "ext", "internal", "external", "perm", "temp", _
                 "stage", "phase", "tier", "primary", "secondary", _
                 "level", "floor", "fl", "zone", "area", "room", "rm", _
                 "wing", "serving", "served", "roof", "mezz", "penthouse", "ph", "podium", _
                 "unit", "equip", "equipment", "system", "systems", "no", "num", "number", _
                 "conc", "concealed", "bldg", "building", "garage", "parkade", "p", "st", _
                 "base", "trk", "track", "mechanical", "mech", _
                 "picture", "clim", "nclim", "heated", "unheated", "steam", "n-heated", "old", "new", "located", _
                 "tower", "cu", "elec", "electrical", "vault", "bsmt", "basement", "hall", "corridor", "closet", "shaft", "lobby"
            Case Else
                out = out & w & " "
        End Select
SkipWord:
    ...
    GetCoreEquipmentName = Trim(out)
End Function
Public Sub ImportAppDataToTemplate()
    Dim pRules As String, pLookups As String, pLearned As String
    Dim wsRules As Worksheet, wsLookups As Worksheet, wsLearned As Worksheet
    Dim streamIn As Object
    Dim allText As String
    Dim lines() As String, parts() As String
    Dim i As Long, r As Long
    Dim outData() As Variant ' Memory array for bulk writing
    
    On Error Resume Next
    Set wsRules = ThisWorkbook.Worksheets("Rules Sheet")
    Set wsLookups = ThisWorkbook.Worksheets("Uniformat RS Means Lookup")
    Set wsLearned = ThisWorkbook.Worksheets("LearnedMappings")
    On Error GoTo 0
    
    pRules = Environ("APPDATA") & "\Uniformat_Rules.txt"
    pLookups = Environ("APPDATA") & "\Uniformat_Lookups.txt"
    pLearned = Environ("APPDATA") & "\Uniformat_Learned.txt"
    ...
    ' --- 1. IMPORT RULES ---
    Application.StatusBar = "Loading AI Vault (1/3): Importing Rules..."
    DoEvents
    
    If Not wsRules Is Nothing And Dir(pRules) <> "" Then
        streamIn.Open
        streamIn.LoadFromFile pRules
        allText = streamIn.ReadText
        streamIn.Close
        
        wsRules.Range("A2:D100000").ClearContents
        lines = Split(Replace(allText, vbCr, ""), vbLf)
        
        ' Size the virtual grid
        ReDim outData(1 To UBound(lines) + 1, 1 To 4)
        r = 1
        
        For i = LBound(lines) To UBound(lines)
            If Len(Trim(lines(i))) > 0 Then
                parts = Split(lines(i), vbTab)
                ' FIX: Filter out ghost header rows
                If UBound(parts) >= 0 Then
                    If LCase(Trim(parts(0))) <> "type" And LCase(Trim(parts(0))) <> "rule type" Then
                        ...
                        If UBound(parts) >= 3 Then outData(r, 4) = parts(3)
                        r = r + 1
                    End If
                End If
            End If
        ...
        If r > 1 Then
            ' Drop the entire grid into Excel instantly
            wsRules.Range("A2").Resize(r - 1, 4).Value = outData
            wsRules.Rows("2:" & r).RowHeight = 12.75
            wsRules.Columns("A:D").AutoFit
            If Not wsRules.AutoFilterMode Then wsRules.Range("A1:D1").AutoFilter
        End If
    End If
    
    ' --- 2. IMPORT LOOKUPS ---
    Application.StatusBar = "Loading AI Vault (2/3): Importing Lookups..."
    DoEvents
    
    If Not wsLookups Is Nothing And Dir(pLookups) <> "" Then
        streamIn.Open
        streamIn.LoadFromFile pLookups
        allText = streamIn.ReadText
        streamIn.Close
        
        ' Clear columns A through H
        wsLookups.Range("A2:H100000").ClearContents
        lines = Split(Replace(allText, vbCr, ""), vbLf)
        
        ' Expand array to 8 columns wide
        ReDim outData(1 To UBound(lines) + 1, 1 To 8)
        r = 1
        ...
        For i = LBound(lines) To UBound(lines)
            If Len(Trim(lines(i))) > 0 Then
                ...
                ' FIX: Filter out ghost header rows
                If UBound(parts) >= 0 Then
                    If LCase(Trim(parts(0))) <> "asset sub type" And LCase(Trim(parts(0))) <> "asset description" Then
                        Dim colIdx As Long
                        ' Map parts directly to columns 1 through 8
                        For colIdx = 0 To UBound(parts)
                            If colIdx < 8 Then outData(r, colIdx + 1) = parts(colIdx)
                        Next colIdx
                        r = r + 1
                    End If
                End If
        ...
        If r > 1 Then
            ' Paste all 8 columns at once
            wsLookups.Range("A2").Resize(r - 1, 8).Value = outData
            wsLookups.Columns("A:H").AutoFit
        End If
    End If
    
    ' --- 3. IMPORT LEARNED MAPPINGS ---
    Application.StatusBar = "Loading AI Vault (3/3): Importing Learned Mappings..."
    DoEvents
    
    If Not wsLearned Is Nothing And Dir(pLearned) <> "" Then
        streamIn.Open
        streamIn.LoadFromFile pLearned
        allText = streamIn.ReadText
        streamIn.Close
        
        wsLearned.Range("A2:E100000").ClearContents
        lines = Split(Replace(allText, vbCr, ""), vbLf)
        
        ReDim outData(1 To UBound(lines) + 1, 1 To 5)
        r = 1
        ...
        For i = LBound(lines) To UBound(lines)
            If Len(Trim(lines(i))) > 0 Then
                ...
                ' FIX: Filter out ghost header rows
                If UBound(parts) >= 0 Then
                    If LCase(Trim(parts(0))) <> "normalizedinput" Then
                        If UBound(parts) >= 0 Then outData(r, 1) = parts(0)
                        If UBound(parts) >= 1 Then outData(r, 2) = parts(1)
                        If UBound(parts) >= 2 Then outData(r, 3) = parts(2)
                        If UBound(parts) >= 3 Then outData(r, 5) = parts(3)
                        r = r + 1
                    End If
                End If
            End If
        ...
        If r > 1 Then
            wsLearned.Range("A2").Resize(r - 1, 5).Value = outData
            wsLearned.Columns("A:E").AutoFit
        End If
    End If
    ...
    MsgBox "AI Master Vault successfully loaded! Your rules and mappings are up to date.", vbInformation, "Import Complete"
End Sub
