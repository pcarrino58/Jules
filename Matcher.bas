...
Public Sub PropagateLearnedChoice(ByVal ws As Worksheet, ByVal sourceRow As Long, ByVal targetSig As String, ByVal targetNorm As String, ByVal chosenOut As String)
    ...
    targetVocabSig = GetVocabSignature(targetNorm)
    
    ' NEW: Get the stripped down core equipment name for the target
    ...
    For r = 2 To lastRow
        If r <> sourceRow Then
            ...
            If Len(Trim$(raw)) > 0 Then
                ...
                rowVocabSig = GetVocabSignature(norm)
                
                ' NEW: Get core name for the current row
                Dim rowCore As String
                rowCore = GetCoreEquipmentName(norm)

                ' 1. Check exact signature match
                ' 2. Check exact text match
                ' 3. Check Omni-Directional 1-word family difference
                ' 4. Check Vocab Signature
                ' 5. NEW: Check Core Equipment Name (ignores directions, stages, and zones)
    ...
    If rowsToUpdate.count > 0 Then
        ...
        For Each k In rowsToUpdate.keys
            ws.Range("A" & CLng(k)).Interior.Color = RGB(201, 231, 167)
...
' 1. FAST BULK WRITE TO TSV (WITH SAFE SHIELD)
    ...
    streamOut.Charset = "utf-8"
    streamOut.Open
    streamOut.WriteText "Row" & vbTab & "Phrase" & vbCrLf
    
    Dim arrIn As Variant, arrStatus As Variant
    arrIn = ws.Range("A1:A" & lastRow).Value2
    arrStatus = ws.Range("C1:C" & lastRow).Value2 ' Grab the status column
...
' THE SHIELD: Determine if the row actually needs AI processing
            ...
            If InStr(1, currentStatus, "Multiple Options", vbTextCompare) > 0 Then needsAI = True ' <--- ADD THIS LINE
            ...
            If needsAI And Len(Trim$(inputPhrase)) > 0 Then
                ...
                streamOut.WriteText i & vbTab & inputPhrase & vbCrLf
            End If
        End If
    Next i
    ...
    If Err.Number <> 0 Or http.Status <> 200 Then
        MsgBox "Failed to connect to AI engine.", vbExclamation
        GoTo Cleanup
    End If
    On Error GoTo 0
...
' Reset base formatting
        ...
        ws.Range("A2:A" & lastRow).Interior.Color = RGB(201, 231, 167) ' Reverts A to Green
        ws.Range("B2:C" & lastRow).Interior.Color = RGB(252, 186, 186) ' Reverts B:C to Pink
        
' Build Pick Lists & Dropdowns
        Dim k As Variant, mArr() As String
        
        ' --- UPDATED AI GUESSES DROPDOWN LOGIC ---
        For Each k In aiGuesses.keys
            ' 1. Split the line-feed string from Python into a VBA array
            mArr = Split(aiGuesses(k), vbLf)
            
            ' 2. Expand the array by 1 to make room for the Reject option
            ReDim Preserve mArr(LBound(mArr) To UBound(mArr) + 1)
            mArr(UBound(mArr)) = "Reject AI Guess"
            
            ' 3. Route through the safe hidden-sheet builder instead of a comma-string
        ...
        ' Apply Yellow Highlighting in a Single Direct Pass (Now includes Column A)
        For Each k In yellowRows.keys
            ws.Range("A" & k & ":C" & k).Interior.Color = RGB(255, 255, 0)
        Next k
        
        ' NEW: Calculate PM Triggers after the bulk run finishes
...
Public Function IsValueFromDropdown(ws As Worksheet, rowNum As Long, chosenVal As String) As Boolean
    ...
    ' 2. Check the raw value in column A
    ...
    ' 3. Re-run the matcher silently in memory for this one row
    Dim outVal As String, confVal As String
    Call GetBestMatchForInput(rawInput, outVal, confVal)

    ' 4. ONLY SKIP LEARNING IF IT IS A MULTIPLE RULES ITEM
    ...
    If InStr(1, confVal, "Multiple Rules", vbTextCompare) > 0 Then
        ...
        For i = LBound(parts) To UBound(parts)
            If LCase$(Trim$(parts(i))) = cleanChosen Then
                IsValueFromDropdown = True ' Returns True to SKIP saving
                Exit Function
            End If
        Next i
    End If
...
Public Sub RunDataPreScrub()
    ...
    Dim lastRow As Long, i As Long
    Dim inputPhrase As String, buildingStr As String, targetCount As Long
    Dim fso As Object
    Dim tempFolder As String, inputFile As String, outputFile As String
    ...
    streamOut.Charset = "utf-8"
    streamOut.Open
    streamOut.WriteText "Row" & vbTab & "Building" & vbTab & "Phrase" & vbCrLf
    
    Dim arrIn As Variant
    Dim arrBldg As Variant
    ...
    For i = 2 To lastRow
        If Not IsError(arrIn(i, 1)) Then
            ...
            If Len(Trim$(inputPhrase)) > 0 Then
                targetCount = targetCount + 1
                If Not IsError(arrBldg(i, 1)) Then buildingStr = CStr(arrBldg(i, 1)) Else buildingStr = ""
                inputPhrase = Replace(Replace(Replace(inputPhrase, vbTab, " "), vbLf, " "), vbCr, " ")
                buildingStr = Replace(Replace(Replace(buildingStr, vbTab, " "), vbLf, " "), vbCr, " ")
                streamOut.WriteText i & vbTab & buildingStr & vbTab & inputPhrase & vbCrLf
            End If
        End If
    Next i
    ...
    If Err.Number <> 0 Or http.Status <> 200 Then
        MsgBox "Failed to connect to AI Pre-Scrubber.", vbExclamation
        GoTo Cleanup
    End If
    On Error GoTo 0
    ...
    wsScrub.Range("E1").Value = "Asset Tag"
    wsScrub.Range("F1").Value = "Location"
    wsScrub.Range("G1").Value = "System Group"
    wsScrub.Range("H1").Value = "Hierarchy"
    wsScrub.Range("I1").Value = "Audit Flag"
    wsScrub.Range("J1").Value = "PM Suitability" '<-- RESTORED 10TH COLUMN
    wsScrub.Range("A1:J1").Font.Bold = True      '<-- EXPANDED TO J
    wsScrub.Range("A1:J1").Interior.Color = RGB(220, 230, 241)
    ...
    If fso.FileExists(outputFile) Then
        ...
        ReDim writeData(1 To UBound(lines), 1 To 10) '<-- EXPANDED TO 10
        ...
        For i = 1 To UBound(lines)
            If Len(lines(i)) > 0 Then
                parts = Split(lines(i), vbTab)
                If UBound(parts) >= 9 Then '<-- INCREASED TO 9 (0-indexed) to catch the PM column
                    writeData(writeCount, 1) = parts(0) ' Row
                    writeData(writeCount, 2) = parts(1) ' Building
                    writeData(writeCount, 3) = parts(2) ' Original
                    writeData(writeCount, 4) = parts(3) ' Scrubbed
                    writeData(writeCount, 5) = parts(4) ' Asset Tag
                    writeData(writeCount, 6) = parts(5) ' Location
                    writeData(writeCount, 7) = parts(6) ' System Group
                    writeData(writeCount, 8) = parts(7) ' Hierarchy
                    writeData(writeCount, 9) = parts(8) ' Audit Flag
                    writeData(writeCount, 10) = parts(9) ' PM Suitability <-- RESTORED
        ...
        If writeCount > 1 Then
            wsScrub.Range("A2").Resize(writeCount - 1, 10).Value2 = writeData '<-- EXPANDED TO 10
            wsScrub.Columns("A:J").AutoFit
            
            Dim lastScrub As Long
            lastScrub = writeCount
            
            wsScrub.Range("I2:I" & lastScrub).FormatConditions.Add Type:=xlTextString, String:="??", TextOperator:=xlContains
            wsScrub.Range("I2:I" & lastScrub).FormatConditions(1).Interior.Color = RGB(255, 235, 156)
            wsScrub.Range("I2:I" & lastScrub).FormatConditions(1).Font.Color = RGB(156, 101, 0)
            
            wsScrub.Range("I2:I" & lastScrub).FormatConditions.Add Type:=xlTextString, String:="Matched", TextOperator:=xlContains
            wsScrub.Range("I2:I" & lastScrub).FormatConditions(2).Interior.Color = RGB(198, 239, 206)
            wsScrub.Range("I2:I" & lastScrub).FormatConditions(2).Font.Color = RGB(0, 97, 0)
            
            wsScrub.Range("I2:I" & lastScrub).FormatConditions.Add Type:=xlTextString, String:="Valid", TextOperator:=xlContains
            wsScrub.Range("I2:I" & lastScrub).FormatConditions(3).Interior.Color = RGB(198, 239, 206)
            wsScrub.Range("I2:I" & lastScrub).FormatConditions(3).Font.Color = RGB(0, 97, 0)
        End If
    End If

    ' =======================================================
    ' FAST STATIC XLOOKUP REPLACEMENT (PM FEASIBILITY)
    ' =======================================================
    ...
    If lastScrubRow >= 2 Then
        ...
        scrubArr = wsScrub.Range("A2:J" & lastScrubRow).Value2
        
        Dim pmArr As Variant
        ReDim pmArr(1 To lastRow - 1, 1 To 1)
        
        Dim rIdx As Long, helperRow As Long
        For rIdx = 1 To UBound(scrubArr, 1)
            helperRow = CLng(val(scrubArr(rIdx, 1))) ' Grabs original Row ID from Col A
            If helperRow >= 2 And helperRow <= lastRow Then
                pmArr(helperRow - 1, 1) = scrubArr(rIdx, 10) ' Grabs PM Suitability from Col J
            End If
        Next rIdx
        
        ' Dump it into Column F (PM Feasibility) instantly
...
Cleanup:
    ...
    wsScrub.Activate
    MsgBox "Pre-Scrub & Audit Complete! Review your clean data and hierarchy flags.", vbInformation, "Data Staging"
...
Public Sub CalculatePMRules(ByVal ws As Worksheet, Optional ByVal showPopup As Boolean = False)
    ...
    ' --------------------------------------------------------
    ' LOAD MANUAL OVERRIDES FROM TRIAGE BOARD (Scrubbed Inputs)
    ' --------------------------------------------------------
    ...
    If Not wsScrub Is Nothing Then
        Dim tLast As Long
        tLast = wsScrub.Cells(wsScrub.Rows.count, "M").End(xlUp).Row
        If tLast >= 2 Then
            Dim tIdx As Long
            For tIdx = 2 To tLast
                Dim tBldg As String, tAsset As String, tChoice As String
                tBldg = UCase(Trim(CStr(wsScrub.Cells(tIdx, "M").Value2)))
                tAsset = LCase(Trim(CStr(wsScrub.Cells(tIdx, "N").Value2)))
                tChoice = Trim(CStr(wsScrub.Cells(tIdx, "O").Value2))
    ...
    Dim arrBldg As Variant, arrAsset As Variant, arrPM As Variant
    ...
    Dim dictKey As String, overrideKey As String
    
    ' --------------------------------------------------------
    ' PASS 1: Count Parents & Children
    ' --------------------------------------------------------
    For i = 2 To lastRow
...
Private Sub GetHierarchyInfo(ByVal assetType As String, ByRef outGroup As String, ByRef outRole As String)
    ...
    If InStr(s, " smoke") > 0 Or InStr(s, " detector") > 0 Or InStr(s, " strobe") > 0 Or InStr(s, " horn") > 0 Or InStr(s, " pull") > 0 Or InStr(s, " bell") > 0 Or InStr(s, " tamper switch") > 0 Or InStr(s, " flow switch") > 0 Or InStr(s, " fire damper") > 0 Or InStr(s, " interface device") > 0 Or InStr(s, " fire box") > 0 Then
...
Public Sub BuildTriageBoard()
    ...
    ' SHIFTED BOARD TO P:R
    wsScrub.Range("P:R").Clear
    wsScrub.Range("P:R").Validation.Delete
    
    wsScrub.Cells(1, "P").Value2 = "BUILDING"
    wsScrub.Cells(1, "Q").Value2 = "AMBIGUOUS FUEL ASSET"
    wsScrub.Cells(1, "R").Value2 = "ASSIGN TO SYSTEM"
    With wsScrub.Range("P1:R1")
        .Font.Bold = True
    ...
    For i = 2 To lastRow
        ' Now looking at Column G for the System Group
        If Trim(CStr(wsScrub.Cells(i, "G").Value2)) = "Ambiguous Fuel System" Then
            wsScrub.Cells(printRow, "P").Value2 = Trim(CStr(wsScrub.Cells(i, "B").Value2))
            wsScrub.Cells(printRow, "Q").Value2 = Trim(CStr(wsScrub.Cells(i, "C").Value2))
            wsScrub.Cells(printRow, "R").Value2 = "Select System..."
            
            With wsScrub.Cells(printRow, "R").Validation
                .Delete
            ...
            wsScrub.Range("P" & printRow & ":R" & printRow).Interior.Color = RGB(255, 242, 204)
            printRow = printRow + 1
        End If
    Next i
    
    If printRow = 2 Then
        wsScrub.Cells(2, "P").Value2 = "-"
        wsScrub.Cells(2, "Q").Value2 = "No ambiguous assets found."
        wsScrub.Cells(2, "R").Value2 = "-"
        wsScrub.Range("P2:R2").Interior.Color = RGB(201, 231, 167)
    End If
    
    wsScrub.Columns("P:R").AutoFit
End Sub
...
Public Sub BuildMissingParentSummary()
    ...
    For i = 2 To lastRow
        bldg = Trim(CStr(ws.Cells(i, "B").Value2))
        ' Now looking at Column I for the Audit Flag
        flag = Trim(CStr(ws.Cells(i, "I").Value2))
    ...
    ws.Cells(1, "K").Value2 = "BUILDING"
    ws.Cells(1, "L").Value2 = "MISSING PARENT ALERT"
    ws.Cells(1, "M").Value2 = "CHILDREN IMPACTED"
    ws.Cells(1, "N").Value2 = "Spacer"
    ...
    If missingDict.count = 0 Then
        ws.Cells(printRow, "K").Value2 = "All Buildings"
        ws.Cells(printRow, "L").Value2 = "All Children Have Parents!"
        ws.Cells(printRow, "M").Value2 = "-"
        ws.Range("K2:M2").Interior.Color = RGB(201, 231, 167)
    Else
        Dim k As Variant
        Dim parts() As String
        For Each k In missingDict.keys
            parts = Split(CStr(k), "|")
            ws.Cells(printRow, "K").Value2 = parts(0)
            ws.Cells(printRow, "L").Value2 = parts(1)
            ws.Cells(printRow, "M").Value2 = missingDict(k)
            
            ws.Range("K" & printRow & ":M" & printRow).Interior.Color = RGB(252, 186, 186)
            printRow = printRow + 1
        Next k
    End If
