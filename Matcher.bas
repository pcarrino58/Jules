...
Public Sub GetBestMatchForInput(ByVal rawInput As String, ByRef outVal As String, ByRef confVal As String)
    ' =========================================================
    ' SAFETY SHIELD TO PREVENT ERROR 91
    ' =========================================================
    ...
    ' -------------------------------------------------------------------------
    ' PRIORITY 5 & 6: SIGNATURE MATCHES (Regex Double-Processing Fix Applied)
    ' -------------------------------------------------------------------------
...
Private Sub EnsureDictionaries()
    ...
    mAliasDict("h p") = "horsepower"            '<-- ADDED: Catches "H.P." after punctuation is stripped
    mAliasDict("dx") = "direct expansion"       '<-- ADDED: Translates DX
...
Public Sub SyncLookupsWithExternal(ByVal wb As Workbook, ByVal sheetName As String)
    ...
    If lastRow >= 2 Then
        For r = 2 To lastRow
            Dim phrase As String
            Dim coreTag As String
            Dim ufCode As String
            
            ' FIX: Using .Text instead of .Value prevents crashes from Excel cell errors (like #N/A)
            phrase = Trim$(ws.Cells(r, "A").text)
            coreTag = Trim$(ws.Cells(r, "B").text)
            ufCode = Trim$(ws.Cells(r, "D").text)
            
            ' Only export if there is an actual phrase
            If Len(phrase) > 0 Then
                ' Pack the phrase, core tag, and RS Means code separated by tabs
                lineStr = phrase & vbTab & coreTag & vbTab & ufCode
...
Public Sub PropagateLearnedChoice(ByVal ws As Worksheet, ByVal sourceRow As Long, ByVal targetSig As String, ByVal targetNorm As String, ByVal chosenOut As String)
    ...
    ' Tell the function it is already normalized to save time
    Dim targetVocabSig As String
    targetVocabSig = GetVocabSignature(targetNorm, True)
    
    Dim targetCore As String
    targetCore = GetCoreEquipmentName(targetNorm)
...
' --- SPEED BOOST 1: Use BOTH Raw and Normalized Words for Fast Pre-Screen ---
    Dim sourceRawText As String
    Dim combinedText As String
    
    sourceRawText = CStr(arrData(sourceRow, 1))
    
    ' Combine raw (e.g., "HUMIDIFIER" or "AHU") with normalized (e.g., "humidification" or "air handling unit")
    combinedText = sourceRawText & " " & targetNorm
    
    Dim targetWords() As String
    targetWords = Split(combinedText, " ")
    ...
    For r = 2 To lastRow
        If r <> sourceRow Then
            ...
            If Len(Trim$(raw)) > 0 Then
                ...
                ' FAST PRE-SCREEN: If the raw text doesn't share a single word
                ' with the target, skip the heavy Regex entirely.
                Dim possibleMatch As Boolean
                possibleMatch = False
                Dim wIdx As Long
                
                For wIdx = LBound(targetWords) To UBound(targetWords)
                    If Len(targetWords(wIdx)) > 2 Then
                        If InStr(1, raw, targetWords(wIdx), vbTextCompare) > 0 Then
                            possibleMatch = True
                            Exit For
                        End If
                    End If
                ...
                ' Force check if target is too short
                If Not possibleMatch And UBound(targetWords) < 1 Then possibleMatch = True
                
                If possibleMatch Then
                    norm = NormalizeAndAlias(raw, mAliasDict)
                    rowSig = BuildLearnSignature(norm)
                    
                    Dim rowVocabSig As String
                    rowVocabSig = GetVocabSignature(norm, True)
                    
                    Dim rowCore As String
                    rowCore = GetCoreEquipmentName(norm)

                    If (Len(targetSig) > 0 And rowSig = targetSig) Or _
                       (norm = targetNorm) Or _
                       IsSameEquipmentFamily(norm, targetNorm) Or _
                       (Len(targetVocabSig) > 0 And rowVocabSig = targetVocabSig) Or _
                       (Len(targetCore) > 0 And rowCore = targetCore) Then
    ...
    ' --- SPEED BOOST 2: BATCH RANGE FORMATTING ---
    If rowsToUpdate.count > 0 Then
        Dim k As Variant
        Dim rngA As Range, rngBC As Range, rngVal As Range
        
        For Each k In rowsToUpdate.keys
            If rngA Is Nothing Then
                Set rngA = ws.Range("A" & CLng(k))
                Set rngBC = ws.Range("B" & CLng(k) & ":C" & CLng(k))
                Set rngVal = ws.Cells(CLng(k), 2)
            Else
                Set rngA = Union(rngA, ws.Range("A" & CLng(k)))
                Set rngBC = Union(rngBC, ws.Range("B" & CLng(k) & ":C" & CLng(k)))
                Set rngVal = Union(rngVal, ws.Cells(CLng(k), 2))
            End If
        Next k
        
        ' Apply formatting all at once instantly
        If Not rngA Is Nothing Then
            rngA.Interior.Color = RGB(201, 231, 167)
            rngBC.Interior.Color = RGB(252, 186, 186)
            
            On Error Resume Next
            Err.Clear
            rngVal.Validation.Delete
            On Error GoTo 0
        End If
    End If
...
SafeExit:
    ...
    Set http = Nothing ' <-- HTTP release added here
End Sub
...
Public Sub RunDataPreScrub()
    ...
    For i = 2 To lastRow
        ...
        mfgStr = Replace(Replace(Replace(mfgStr, vbTab, " "), vbLf, " "), vbCr, " ")
        modelStr = Replace(Replace(Replace(modelStr, vbTab, " "), vbLf, " "), vbCr, " ")
        serialStr = Replace(Replace(Replace(serialStr, vbTab, " "), vbLf, " "), vbCr, " ")
    ...
    ReDim writeData(1 To UBound(lines), 1 To 17)
    Dim writeCount As Long: writeCount = 1
    ...
    For i = 1 To UBound(lines)
        If Len(lines(i)) > 0 Then
            ...
            If UBound(parts) >= 16 Then
                Dim c As Long
                For c = 0 To 16
                    writeData(writeCount, c + 1) = parts(c)
                Next c
    ...
    If writeCount > 1 Then
        wsScrub.Range("A2").Resize(writeCount - 1, 17).Value2 = writeData
        wsScrub.Columns("A:Q").AutoFit
        
        Dim lastScrub As Long: lastScrub = writeCount
...
Private Function GetCoreEquipmentName(ByVal text As String) As String
    ...
    For i = LBound(parts) To UBound(parts)
        ...
        ' Strip out single letters (A, B, C) and pure numbers
        If Len(w) > 1 Then
            If Not IsNumeric(w) Then
                ...
                Select Case w
                    Case "north", "south", "east", "west", "ne", "nw", "se", "sw", _
                         ...
                         "wing", "serving", "served", "roof", "mezz", "penthouse", "ph", "podium"
                        ' Skip it (do not add to core name)
                    Case Else
                        out = out & w & " "
                End Select
            End If
