...
def load_knowledge_base():
    ...
    # 1. Locate the newest Excel template (ignoring MS Office ghost files)
    ...
    if excel_files:
        ...
        try:
            print("Reading Lookups from Column B...")
    ...
    # 2. Overlay live text files from %APPDATA% (VBA Sync Updates / Fallback)
    ...
    if os.path.exists(lookups_file):
        try:
            ...
            with open(lookups_file, "r", encoding="utf-8-sig") as f:
                for line in f:
                    parts = line.strip('\n').split('\t')
                    clean_phrase = parts[1].strip() if len(parts) >= 2 else parts[0].strip()
...
def prescrub_text(raw_text, use_whitelist=True, current_valid_words=None, current_stemmed_whitelist=None, current_pattern=None):
    ...
    clean = re.sub(r'\b(?:blue|green|yellow|red|orange|purple|brown|grey|gray|white|black|east|west|north|south|behind|beside|across|inside|freight|expansion|retail|tenant|dock|bay)\b', '', clean)
...
def run_ai_batch_processing(items, is_batch_file=False):
    ...
    for row in items:
        ...
        for messy_term, rule_tokens, clean_term in local_compiled_expert_rules:
            # Check exact phrase/abbreviation match with word boundaries instead of un-ordered subsets
            pattern = r'(?<![a-z0-9])' + re.escape(messy_term) + r'(?:s|es)?(?![a-z0-9])'
            if re.search(pattern, original_clean, re.IGNORECASE):
