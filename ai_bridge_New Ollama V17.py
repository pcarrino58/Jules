...
HIERARCHY_RULES = [
    {"group": "Life Safety / Fire", "parents": [r'\bfire alarm control', r'\bfire panel', r'\bfacp\b', r'\bannunciator', r'\bmain fire'], "children": [r'\bsmoke', r'\bdetector', r'\bstrobe', r'\bhorn', r'\bpull', r'\bbell', r'\btamper switch', r'\bflow switch', r'\bfire damper', r'\binterface device', r'\bfire box']},
...
COMPILED_HIERARCHY_RULES = [
    {
        "group": rule["group"],
        "parents": [re.compile(r'(?<![a-z0-9])' + p.replace(r'\b', '') + r'(?![a-z0-9])', re.IGNORECASE) for p in rule["parents"]],
        "children": [re.compile(r'(?<![a-z0-9])' + c.replace(r'\b', '') + r'(?![a-z0-9])', re.IGNORECASE) for c in rule["children"]]
    }
                    ...
                    if parts:
                        ...
                        if clean_phrase and clean_phrase.lower() != "asset sub type":
                            ...
                            if lower_phrase not in t_canonical_casing:
                                paired.append((stripped_phrase, clean_phrase))
                                t_canonical_casing[lower_phrase] = clean_phrase
                                for word in stripped_phrase.split(): t_uniformat_valid_words.add(word)
        except Exception:
            pass
...
def identify_equipment_from_model(model_num, serial_num, raw_phrase):
    if not isinstance(model_num, str) or not model_num.strip() or model_num.lower() == 'nan':
        return {"identity": "", "consumables": ""}

    url = "http://localhost:11434/api/generate"
    prompt = f"""
You are an expert equipment appraiser and mechanical engineer.
Analyze the following manufacturer Model Number.

MODEL NUMBER: "{model_num}"
SERIAL NUMBER: "{serial_num}"
CLIENT LABEL: "{raw_phrase}"

INSTRUCTIONS:
1. Determine the specific manufacturer and real-world equipment type (e.g., "Carrier AquaForce Water-Cooled Chiller").
2. Decode the Model Number and Serial Number to extract all available specifications (e.g., Tonnage, Voltage, Phase, Capacity, Refrigerant, Year of Manufacture).
3. If you know the specific standard consumables (e.g., Filter sizes, Belts), list them as well.
4. If you cannot decode any specs or consumables, output "No specs decoded".

Respond ONLY in valid JSON with these exact keys:
{{
    "equipment_identity": "Clean Asset Type Here",
    "specs_and_consumables": "Tonnage: 50 | Voltage: 460v | Refrigerant: R-410A | Filters: 20x20x2"
}}
"""
    payload = {
        "model": "qwen2.5:7b",
    ...
    try:
        response = http_session.post(url, json=payload, timeout=45)
        if response.status_code == 200:
            raw_res = response.json().get("response", "").strip()
            ...
            return {
                ...
                "consumables": data.get("specs_and_consumables", "").strip()
            }
    except Exception as e:
        print(f"Ollama Identity Error: {e}")

    return {"identity": "LLM Offline/Timeout", "consumables": ""}
...
def run_ai_batch_processing(items, is_batch_file=False):
    ...
    for row in items:
        if is_batch_file:
            ...
            s_num = str(row.get('Serial', '')).strip()
        else:
            m_num = str(row.get('model', '')).strip()
            o_phr = str(row.get('phrase', '')).strip()
            s_num = str(row.get('serial', '')).strip()
        ...
        if m_num:
            cache_key = f"{m_num}|{o_phr}"
            with llm_cache_lock:
                if cache_key not in global_llm_cache:
                    unique_queries[cache_key] = (m_num, s_num, o_phr)

    if unique_queries:
        print(f"\n--- Batching {len(unique_queries)} unique models to Ollama (Multithreaded) ---")
        def fetch_and_cache(c_key, m, s, p):
            llm_data = identify_equipment_from_model(m, s, p)
            with llm_cache_lock:
                global_llm_cache[c_key] = llm_data
            print(f"Resolved Model: {m} -> {llm_data.get('identity', '')}")

        with ThreadPoolExecutor(max_workers=2) as executor:
            futures = [executor.submit(fetch_and_cache, k, m, s, p) for k, (m, s, p) in unique_queries.items()]
            for future in as_completed(futures): pass
        print("--- Batch complete! Compiling final data... ---\n")
    ...
    for row in items:
        ...
        if model_num:
            cache_key = f"{model_num}|{original_phrase}"
            with llm_cache_lock:
                llm_data = global_llm_cache.get(cache_key, {"identity": "", "consumables": ""})

            if not llm_data.get("identity"):
                llm_data = identify_equipment_from_model(model_num, serial_num, original_phrase)
                with llm_cache_lock:
                    global_llm_cache[cache_key] = llm_data

            llm_identity = llm_data.get("identity", "")
            llm_consumables = llm_data.get("consumables", "")

            if llm_identity and "Offline" not in llm_identity:
                resolved_phrase = llm_identity
...
def prescrub_file():
    ...
    for row in records:
        ...
        if model_num:
            ...
            with llm_cache_lock:
                if cache_key not in global_llm_cache:
                    unique_queries[cache_key] = (model_num, serial_num, original_phrase)
    ...
    for row in records:
        ...
        processed_data.append({
            ...
            "LLM Identity": llm_identity,
            "Consumables & Specs": llm_consumables,
            "Asset Tag": asset_id,
    ...
    df_processed = df_processed[["Row", "Building", "Original Phrase", "Scrubbed Phrase", "LLM Identity", "Consumables & Specs", "Asset Tag", "Location", "System Group", "Hierarchy", "Audit Flag", "PM Suitability"]]
