...
Public Sub EmergencyClearSheet()
    ...
    ' Wipes everything but the headers
    Worksheets("Helper Sheet").Range("A2:C100000").ClearContents
    Worksheets("Helper Sheet").Range("B2:C100000").Validation.Delete
    Worksheets("Helper Sheet").Range("B2:C100000").Interior.Color = xlNone
...
Private Function GetOrCreateListRange(ByVal wsList As Worksheet, ByVal listName As String, ByVal itemCount As Long) As Range
    ...
    ' FIX 1: Prevent row 0 errors if an empty list is passed
    ...
    ' FIX 2: Prevent Error 1004 by clearing the sheet if it hits Excel's row limit
    If endRow > wsList.Rows.count Then
        wsList.Cells.Clear
        startRow = 1
        endRow = itemCount
    End If
...
Public Sub SyncLookupsWithExternal(ByVal wb As Workbook, ByVal sheetName As String)
    ...
    If lastRow >= 2 Then
        For r = 2 To lastRow
            ...
            phrase = Trim$(CStr(ws.Cells(r, "A").Value))
            ufCode = Trim$(CStr(ws.Cells(r, "D").Value))
            
            ' Only export if there is an actual phrase
            If Len(phrase) > 0 Then
                ' Pack both the phrase and the RS Means code separated by a tab
                lineStr = phrase & vbTab & ufCode
...
Public Sub RunDataPreScrub()
    ...
    If fso.FileExists(outputFile) Then
        ...
        For i = 1 To UBound(lines)
            If Len(lines(i)) > 0 Then
                parts = Split(lines(i), vbTab)
                If UBound(parts) >= 9 Then '<-- INCREASED TO 9 (0-indexed)
                    ...
                    writeData(writeCount, 6) = parts(5) ' Location (NEW)
                    ...
                    writeData(writeCount, 10) = parts(9) ' PM Suitability <-- NEW
