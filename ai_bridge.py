...
# ==========================================
# ROUTE 1: The "Thinking" Lookup Function
# ==========================================
...
def lookup():
    ...
    try:
        # STEP 1: THE LIBRARIAN (Get top 10 fuzzy matches)
        ...
        # Extract those 10 options into a clean list
        top_10_options = results['documents'][0]
        
        # Remove duplicates just in case the vault has multiples
        unique_options = list(set(top_10_options))
        options_string = "\n".join(f"- {opt}" for opt in unique_options)

        # STEP 2: THE MECHANIC (Reasoning Layer)
        system_prompt = f"""You are an expert mechanical facility asset classifier.
Your ONLY job is to identify the primary piece of equipment in a messy vendor string and match it to the absolute best option from the provided list.

CRITICAL ENGINEERING RULES YOU MUST FOLLOW:

1. THE "LOCATION IGNORE" RULE:
Ignore building names, room numbers, and project codes. You MUST specifically ignore location tags like MECH, ELEC, ROOF, BASEMENT, PENTHOUSE, or ROOM. Focus purely on the machinery.

2. THE "HYBRID EQUIPMENT" RULE (Heavy beats Light):
If a string contains two different pieces of equipment or a component, you must identify the primary, heavier asset and ignore the secondary asset.
- Chiller vs Condenser: If a string has both (e.g., "Condensing-Chiller"), ALWAYS output the Chiller.
- Boiler vs Pump: If a string has both (e.g., "Boiler feed pump"), ALWAYS output the Boiler.
- Asset vs Controller/Detector: If it says "Sump Pump Controller" or "Fire Alarm Panel", ALWAYS output the main Pump or System.

3. STRICT OUTPUT RULES:
- Pick the ONE option from the Available Options list that best represents the primary equipment.
- Return ONLY the exact text of that option. Do not include any explanations, reasoning, or conversational text.
...
# ==========================================
# ROUTE 2: The Universal "Learn" Function 
# ==========================================
