...
# --- CUSTOM CACHING & DDG RATE LIMITING ---
prescrub_cache = {}
prescrub_cache_lock = threading.Lock()

ddg_lock = threading.Lock()
last_ddg_time = 0.0
ddg_rate_limited_until = 0.0
...
def recompile_custom_rules_global():
    ...
    # Wipe the pre-scrub cache so new rules are actually applied
            ...
            if "Uniformat RS Means Lookup" in xls.sheet_names:
                ...
                for row in df_lookups_records:
                    if 0 in row and pd.notna(row[0]):
                        val = str(row[0]).strip()
                        uf_val = str(row.get(3, "")).strip() if pd.notna(row.get(3, "")) else "" 
                        ...
                        if clean_phrase:
                            ...
                            # --- ADD THESE TWO LINES TO FIX THE DICTIONARY ---
...
# Custom Caching Wrapper to safely bypass unhashable variables and prevent RAM bloat
...
def validate_model_and_serial(manufacturer, model_num, serial_num):
    ...
    def is_real(val):
        if not val:
            return False
        v_low = val.lower()
        # Filter out placeholders, single digits, and zeros
...
# ==========================================
# TIER 1: GLOBALLY CACHED MODEL LOOKUP (Two-Stage RAG)
# ==========================================
def extract_model_data_global(brand_context, clean_model, base_phrase, client_uf=""):
    global ddg_rate_limited_until, last_ddg_time
    url = "http://localhost:11434/api/generate"
    ...
    # ---------------------------------------------------------
    # STAGE 1: Pure Nomenclature Decode
    # ---------------------------------------------------------
    prompt_stage_1 = f"""You are a Master Mechanical Engineer and HVAC Equipment Specialist. 
...
{{
    ...
    "debug_source": "Nomenclature Decode"
}}
"""
    
    data = call_ollama(prompt_stage_1)
    ...
    # ---------------------------------------------------------
    # STAGE 2: Web Search Fallback (Thread-Safe Rate Limit)
    # ---------------------------------------------------------
    if tech_specs in ["DATA_MISSING", ""] and clean_model and len(clean_model) >= 4:
        search_context = ""
        try:
            with ddg_lock:
                if time.time() < ddg_rate_limited_until:
                    raise Exception("rate_limit_backoff")
                
                elapsed = time.time() - last_ddg_time
                if elapsed < 2.0:
                    time.sleep(2.0 - elapsed)
                last_ddg_time = time.time()
                
            # Filter out DATA_MISSING from search keywords just in case
            search_identity = identity if identity not in ["DATA_MISSING", base_phrase, ""] else ""
            search_query_parts = [brand_context, clean_model, search_identity, "specifications"]
            ...
            if not results:
                print(f"    [DDG] ⚠️ 0 results for: {query}")
            else:
                print(f"    [DDG] 🌐 Found {len(results)} pages for: {query}")
                snippets = [f"- {r.get('title', '')}: {r.get('body', '')}" for r in results]
                search_context = "\n".join(snippets)
                
        except Exception as e:
            if "rate" in str(e).lower() or "202" in str(e) or "rate_limit_backoff" in str(e):
                with ddg_lock:
                    if time.time() >= ddg_rate_limited_until:
                        ddg_rate_limited_until = time.time() + 60.0
                        print("\n[WARNING] DuckDuckGo Rate Limit Hit! Engaging 60-second safety lock.\n")
            else:
                print(f"    [DDG] 💥 Search Error: {e}")

        if search_context:
            prompt_stage_2 = f"""You are an HVAC mechanical engineer. Extract specifications from the WEB CONTEXT for this equipment.

EQUIPMENT: {brand_context} {clean_model} ({identity})
WEB CONTEXT:
{search_context}

INSTRUCTIONS:
1. Extract Voltage, Drive Type, Capacity, and Heating source if explicitly stated in the context.
2. If the context does not specify these details, output "DATA_MISSING". Do NOT use placeholders like "[X]".

Respond ONLY in valid JSON:
{{
    "equipment_identity": "{identity}",
    "technical_specs": "Voltage: [value or DATA_MISSING] | Type: [value] | Drive: [value or DATA_MISSING] | Heating: [value or DATA_MISSING]",
    "consumables": "DATA_MISSING"
}}
"""
            data_stage_2 = call_ollama(prompt_stage_2)
            if data_stage_2:
                raw_specs = data_stage_2.get("technical_specs", "DATA_MISSING").strip()
                if "[X]" in raw_specs or "[value" in raw_specs:
                    tech_specs = "DATA_MISSING"
                else:
                    tech_specs = raw_specs
                consumables = data_stage_2.get("consumables", "DATA_MISSING").strip()
                print(f"    [LLM Stage 2] Web RAG Specs: {tech_specs}")

    # THE FIX: Catch bad identities and forcefully revert them back to your valid Original Phrase
...
def decode_serial_data_local(brand_context, clean_serial):
    ...
    try:
        # 1. CARRIER / BRYANT / PAYNE (Format: 1014E12345 -> Week 10, Year 2014)
        ...
        # 2. TRANE / AMERICAN STANDARD (Format: 1813E1234 -> Year 2018, Week 13)
        ...
        # 3. RHEEM / RUUD (Format: W352012345 -> Week 35, Year 2020)
        ...
        # 4. LENNOX (Format: 5815G12345 -> Plant 58, Year 2015, Month G)
        if "lennox" in brand:
            match = re.search(r'^\d{2}(\d{2})[A-Z]', serial)
            if match:
                year = match.group(1)
        ...
        pass # If regex fails, gracefully fall down to the LLM fallback below

    # =================================================================
    # FALLBACK: If brand is unknown, let the LLM try to decode it
    # =================================================================
    url = "http://localhost:11434/api/generate"
        ...
        try:
            ...
            for row in records:
                ...
                if clean_model or brand_context or base_phrase:
                    if clean_model:
                        model_key = f"{brand_context.upper()}|{clean_model.upper()}|{base_phrase.upper()}"
                    elif brand_context or base_phrase:
                        model_key = f"{brand_context.upper()}|{base_phrase.upper()}"
                    else:
                        continue
                ...
                if clean_serial and brand_context:
                    if any(b in brand_context.lower() for b in MAJOR_SERIAL_BRANDS):
                        serial_key = f"{brand_context.upper()}|{clean_serial.upper()}"
                        with serial_cache_lock:
            ...
            for row in records:
                ...
                # CUT THE WIRE FIX: Force the scrubber to strictly use the client's original phrase
                resolved_phrase = base_phrase
                
                scrubbed = prescrub_text_cached(resolved_phrase, use_whitelist=True, current_valid_words=local_valid_words, current_stemmed_whitelist=local_stemmed_whitelist, current_pattern=local_master_rule_pattern)
                ...
                if match_found: eval_phrase = match_found 
                
                sys_group, hierarchy = categorize_asset(eval_phrase)
                pm_suitability = determine_pm_suitability(eval_phrase)

                # ========================================================
                # NEW ALIGNMENT FIX FOR COLUMN E
                # ========================================================
                # 1. If the scrubber found a valid match, force Column E to align perfectly.
                # 2. If the scrubber failed ("data missing"), ONLY THEN use the LLM's guess.
                bad_evals = ["no good match", "data missing", "", "skip"]
                if eval_phrase.lower() in bad_evals and llm_identity and llm_identity.lower() not in bad_evals:
                    display_identity = llm_identity.title()
                else:
                    display_identity = eval_phrase.title()
